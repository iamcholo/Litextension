...
class LitExtension_CartServiceMigrate_Model_Process{
    ...
    protected function _checkAndGetOptionValue($attribute_option, $store_id){
        ...
        $options = Mage::getModel("eav/config")
            ->getAttribute("catalog_product", LitExtension_CartServiceMigrate_Model_Cart::MANUFACTURER_CODE)
            ->setStoreId($store_id)
            ->getSource()
            ->getAllOptions(false);
        ...
        try{
            ...
            foreach ($carts as $item) {
                if(isset($item['product_id'])){
                    if(LitExtension_CartServiceMigrate_Model_Custom::PRODUCT_REPORT){
                        $this->_addProductIsView($item['product_id'], $orderData['customer_id']);
                    }
                    if(LitExtension_CartServiceMigrate_Model_Custom::PRODUCT_BESTSELLER){
                ...
                if (isset($item['children_item'])) {
                    foreach ($item['children_item'] as $child_item) {
                        $childItem = Mage::getModel("sales/order_item")
                            ->setStoreId($store_id);
                        $childItem->addData($child_item);
                        $childItem->setParentItem($orderItem);
                        $allChild[] = $childItem;
                    }
                }
