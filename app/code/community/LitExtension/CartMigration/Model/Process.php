...
class LitExtension_CartMigration_Model_Process
{
    ...
    protected function _clearProducts($cart, $notice)
    {
        ...
        $collection = Mage::getModel('catalog/product')
            ->getCollection()
            ->addWebsiteFilter($notice['config']['website_id'])
    ...
    protected function _clearCustomers($cart, $notice)
    {
        ...
        $collection = Mage::getModel('customer/customer')
            ->getCollection()
            ->addFieldToFilter('website_id', $notice['config']['website_id'])
            ->setPageSize($notice['clear_info']['limit'])
            ->setCurPage(1);
        if (!count($collection)) {
    ...
    protected function _clearOrders($cart, $notice)
    {
        ...
        $collection = Mage::getModel('sales/order')
            ->getCollection()
            ->addFieldToFilter('store_id', array_values($notice['config']['languages']))
            ->setPageSize($notice['clear_info']['limit'])
            ->setCurPage(1);
        if (!count($collection)) {
    ...
    protected function _clearReviews($cart, $notice)
    {
        ...
        $collection = Mage::getModel('review/review')
            ->getCollection()
            ->addStoreFilter(array_values($notice['config']['languages']))
            ->setPageSize($notice['clear_info']['limit'])
            ->setCurPage(1);
        if (!count($collection)) {
    ...
    protected function _clearTaxRules($cart, $notice)
    {
        ...
        if (!count($collection)) {
            ...
            foreach ($collection as $tax_rule) {
                try {
                    $tax_rule->getCalculationModel()->deleteByRuleId($tax_rule->getId());
                    $tax_rule->delete();
                } catch (LitExtension_CartMigration_Exception $e) {
                    $response['result'] = 'error';
    ...
    protected function _clearPages($cart, $notice)
    {
        if (!$notice['config']['import']['pages']) {
            return array(
                'result' => 'process',
        ...
        if (!count($collection)) {
            $response['result'] = 'process';
            $response['function'] = '_clearBlocks';
        } else {
            foreach ($collection as $cms_page) {
                try {
                    ...
                    $response['result'] = 'error';
                    $response['msg'] = "CMS Page Id = {$cms_page->getId()} delete failed. Error: " . $e->getMessage();
                    break;
                } catch (Exception $e) {
                    $response['result'] = 'error';
                    $response['msg'] = "CMS Page Id = {$cms_page->getId()} delete failed. Error: " . $e->getMessage();
    ...
    protected function _clearBlocks($cart, $notice)
    {
        if (!$notice['config']['import']['blocks']) {
            return array(
                'result' => 'process',
        ...
        if (!count($collection)) {
            $response['result'] = 'process';
            $response['function'] = '_clearTransactions';
        } else {
            foreach ($collection as $cms_block) {
                try {
                    $cms_block->delete();
                } catch (LitExtension_CartMigration_Exception $e) {
                    $response['result'] = 'error';
                    $response['msg'] = "CMS Block Id = {$cms_block->getId()} delete failed. Error: " . $e->getMessage();
                    break;
                } catch (Exception $e) {
                    $response['result'] = 'error';
                    $response['msg'] = "CMS Block Id = {$cms_block->getId()} delete failed. Error: " . $e->getMessage();
    ...
    protected function _clearTransactions($cart, $notice) {
        if (!$notice['config']['import']['transactions']) {
            return array(
                'result' => 'process',
        ...
        $collection = Mage::getModel('core/email_template')
            ->getCollection()
            ->setPageSize($notice['clear_info']['limit'])
            ->setCurPage(1);
        if (!count($collection)) {
            $response['result'] = 'process';
            $response['function'] = '_clearRules';
        } else {
            foreach ($collection as $transaction) {
                try {
                    ...
                    $response['result'] = 'error';
                    $response['msg'] = "Transaction Id = {$transaction->getId()} delete failed. Error: " . $e->getMessage();
                    break;
                } catch (Exception $e) {
                    $response['result'] = 'error';
                    $response['msg'] = "Transaction Id = {$transaction->getId()} delete failed. Error: " . $e->getMessage();
    ...
    protected function _clearRules($cart, $notice) {
        if (!$notice['config']['import']['rules']) {
            return array(
                'result' => 'process',
        ...
        if (!count($collection)) {
            $response['result'] = 'process';
            $response['function'] = '_clearCartrules';
        } else {
            foreach ($collection as $rule) {
                try {
                    ...
                    $response['result'] = 'error';
                    $response['msg'] = "Rule Id = {$rule->getId()} delete failed. Error: " . $e->getMessage();
                    break;
                } catch (Exception $e) {
                    $response['result'] = 'error';
                    $response['msg'] = "Rule Id = {$rule->getId()} delete failed. Error: " . $e->getMessage();
    ...
    protected function _clearCartrules($cart, $notice) {
        if (!$notice['config']['import']['cartrules']) {
            return array(
                'result' => 'success',
                'function' => ''
            );
        ...
        if (!count($collection)) {
            ...
            foreach ($collection as $rule) {
                try {
                    ...
                    $response['result'] = 'error';
                    $response['msg'] = "CartRule Id = {$rule->getId()} delete failed. Error: " . $e->getMessage();
                    break;
                } catch (Exception $e) {
                    $response['result'] = 'error';
                    $response['msg'] = "CartRule Id = {$rule->getId()} delete failed. Error: " . $e->getMessage();
    ...
    public function cms($data)
    {
        ...
        try {
            $cmsPage->save();
            $response['result'] = 'success';
            $response['mage_id'] = $cmsPage->getId();
        } catch (LitExtension_CartMigration_Exception $e) {
            $response['result'] = 'error';
            $response['msg'] = $e->getMessage();
        } catch (Exception $e) {
    ...
    protected function _checkAndGetOptionValue($attribute_option, $store_id){
        ...
        $options = Mage::getModel("eav/config")
            ->getAttribute("catalog_product", LitExtension_CartMigration_Model_Cart::MANUFACTURER_CODE)
            ->setStoreId($store_id)
            ->getSource()
            ->getAllOptions(false);
    ...
    public function product($data){
        ...
        try{
            ...
            if($image_import_path && isset($image_import_path['path']) && file_exists($image_import_path['path'])){
                $exclude = isset($image_import_path['disabled']) ? (bool)$image_import_path['disabled'] : false;
                $_product->addImageToMediaGallery($image_import_path['path'] ,array('thumbnail', 'small_image', 'image'), true, $exclude, $image_import_path['label']);
            }
            if($galleries){
                foreach($galleries as $gallery){
                    if(file_exists($gallery['path'])){
                        $exclude = isset($gallery['disabled']) ? (bool)$gallery['disabled'] : false;
                        $_product->addImageToMediaGallery($gallery['path'], array(), true, $exclude, $gallery['label']);
                    }
                }
            }
            $_product->save();
        ...
        if($response['result'] == 'success' && $multi_store && !empty($multi_store)){
            foreach($multi_store as $store_data){
                try{
                    $product = Mage::getModel('catalog/product')->setStoreId(0)->load($response['mage_id']);
                    $product->addData($store_data);
                    $product->setStoreId($store_data['store_id']);
                    $product->save();
                } catch(LitExtension_CartMigration_Exception $e){
                    //do nothing
                } catch(Exception $e){
    ...
    public function order($data, $pre_ord){
        ...
        try{
            ...
            if (!isset($paymentData['method']) || !$paymentData['method']) {
                $orderPayment = Mage::getModel('sales/order_payment')
                    ->setStoreId($store_id)
                    ->setMethod('checkmo');
                $order->setPayment($orderPayment);
            } else {
                $allMethod = Mage::getStoreConfig('payment', $store_id);
                $orderPayment = Mage::getModel('sales/order_payment');
                if (isset($allMethod[$paymentData['method']])) {
                    $paymentMethod = $orderPayment->setStoreId($store_id)->setData($paymentData);
                } else {
                    $paymentMethod = $orderPayment->setStoreId($store_id)->setMethod('checkmo');
                }
                $order->setPayment($paymentMethod);
            }
            foreach ($carts as $item) {
                if(isset($item['product_id'])){
                    if(Mage::getStoreConfig('lecamg/general/report')){
                        $this->_addProductIsView($item['product_id'], $orderData['customer_id']);
                    }
                    if(Mage::getStoreConfig('lecamg/general/bestseller')){
                ...
                if (isset($item['children_item'])) {
                    $allChild = array();
                    foreach ($item['children_item'] as $child_item) {
                        $chilItem = Mage::getModel('sales/order_item')
                            ->setStoreId($store_id);
                        $chilItem->addData($child_item);
                        $chilItem->setParentItem($orderItem);
                        $allChild[] = $chilItem;
                    }
                }
    ...
    public function cart($data){
        ...
        try{
            $quote = Mage::getModel('sales/quote')
                ->setStoreId($store_id);
            $billingAddress = Mage::getModel('sales/quote_address')->addData($address_billing);
            $shippingAddress = Mage::getModel('sales/quote_address')->addData($address_shipping);
            ...
            if (isset($paymentData['method']) && $paymentData['method']) {
                ...
                if (isset($allMethod[$paymentData['method']])) {
                    $paymentMethod = $quotePayment->setData($paymentData);
                } else {
                    $paymentMethod = $quotePayment->setMethod('checkmo');
                }
            ...
            foreach ($items as $item) {
                $quoteItem = Mage::getModel('sales/quote_item')
                    ->setStoreId($store_id)
                    ->setQuote($quote);
                $quoteItem->addData($item);
                $quoteItem->setOptions($item['item_options']);
                if (isset($item['children_item'])) {
                    $allChild = array();
                    foreach ($item['children_item'] as $child_item) {
                        $chilItem = Mage::getModel('sales/quote_item')
                            ->setStoreId($store_id)
                            ->setQuote($quote);
                        $chilItem->addData($child_item);
                        $chilItem->setParentItem($quoteItem);
                        $chilItem->setOptions($child_item['item_options']);
                        $allChild[] = $chilItem;
                    }
                }
...
//        $groupSets = array();
//        if (isset($data['attribute_group_set'])) {
//            $groupSets = $data['attribute_group_set'];
//            unset($data['attribute_group_set']);
//        }
        ...
        try{
            ...
            $setInfo = Mage::getResourceModel('eav/entity_attribute_set')->getSetInfo(array($attr_id));
            $attributeSetIds = array_keys($setInfo[$attr_id]);
        }catch (Exception $e){
            return false;
        }
        if($data['attribute_set_id'] && !is_array($data['attribute_set_id'])){
            $setup = Mage::getModel('eav/entity_setup', 'core_setup');
            if (!in_array($data['attribute_set_id'], $attributeSetIds)) {
                try{
        ...
        if($data['attribute_set_id'] && is_array($data['attribute_set_id'])){
            $setup = Mage::getModel('eav/entity_setup', 'core_setup');
            foreach ($data['attribute_set_id'] as $attr_set_id) {
                if (!in_array($attr_set_id, $attributeSetIds)) {
                    try{
                        $setup->addAttributeToSet(Mage_Catalog_Model_Product::ENTITY, $attr_set_id, '', $attr_id);
                    }catch (Exception $e){}
                }
            }
        }
//        foreach ($groupSets as $group_set) {
//            $setup = Mage::getModel('eav/entity_setup', 'core_setup');
//            try {
//                $setup->addAttributeGroup(Mage_Catalog_Model_Product::ENTITY, $group_set['attribute_set_id'], $group_set['attribute_group_name'], $group_set['sort_order']);
//                $setup->addAttributeToGroup(Mage_Catalog_Model_Product::ENTITY, $group_set['attribute_set_id'], $group_set['attribute_group_name'], $attr_id);
//            } catch (Exception $ex) {
//
//            }
//        }
        ...
        try{
            ...
            /*if($comment['updated_at']){
                $order->setUpdatedAt($comment['updated_at'])
                    ->setStatus($comment['status']);
                if(isset($comment['state']) && $comment['state']){
                    $order->setData('state', $comment['state']);
                }
            }*/
            $order->save();
        } catch(Exception $e){
    ...
    public function ordersInvoice($order_id, $data) {
        $invoiceData = $data['invoice'];
        $itemData = $data['item'];
        $commentData = $data['comment'];
        $store_id = $invoiceData['store_id'];
        try {
            $increment_id = Mage::getSingleton('eav/config')->getEntityType('invoice')->fetchNewIncrementId($store_id);
            ...
            foreach ($itemData as $id => $item) {
                $invoice_item = Mage::getModel('sales/order_invoice_item')
                        ->setOrderItem($order_item[$id]);
                $invoice_item->addData($item);
                $invoice->addItem($invoice_item);
            }
            $invoice->addData($invoiceData);
            $invoice->save();
            foreach ($commentData as $comment) {
                $invoice_comment = Mage::getModel('sales/order_invoice_comment')
                        ->setStoreId($store_id);
                $invoice_comment->addData($comment);
                $invoice->getCommentsCollection()->addItem($invoice_comment);
            }
            $invoice->save();
        } catch (Exception $ex) {
    ...
    public function productDownloadLink($data) {
