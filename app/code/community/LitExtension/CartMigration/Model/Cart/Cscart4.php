 ...
 */
class LitExtension_CartMigration_Model_Cart_Cscart4 extends LitExtension_CartMigration_Model_Cart {
    ...
    public function displayConfig() {
        ...
        $default_cfg = $this->_getDataImport($this->_getUrlConnector('query'), array(
            'serialize' => true,
            'query' => serialize(array(
                "languages" => "SELECT cfg.*, lg.* FROM _DBPRF_settings_objects AS cfg LEFT JOIN _DBPRF_languages AS lg ON lg.lang_code = cfg.value WHERE cfg.name = 'frontend_default_language' OR cfg.name = 'customer_default_language'",
    ...
    public function convertCategory($category, $categoriesExt) {
        ...
        if ($cat_img) {
            ...
            if ($cat_img_path && $img_path = $this->downloadImage($this->getUrlSuffix($this->_notice['config']['image_category']), $cat_img_path, 'catalog/category')) {
    ...
    protected function _getProductsExtQuery($products) {
        ...
        $ext_query = array(
            ...
            'product_features_values' => "SELECT * FROM _DBPRF_product_features_values WHERE product_id IN {$pro_ids_query}",
            ...
            'product_options_inventory' => "SELECT * FROM _DBPRF_product_options_inventory WHERE product_id IN {$pro_ids_query}",
            ...
            'tag_links' => "SELECT a.*, b.* FROM _DBPRF_tag_links a LEFT JOIN _DBPRF_tags b ON a.tag_id = b.tag_id WHERE a.object_type = 'P' AND a.object_id IN {$pro_ids_query}"
        );
        return $ext_query;
    }
    ...
    protected function _getProductsExtRelQuery($products, $productsExt) {
        ...
        $productIds = $this->duplicateFieldValueFromList($products['object'], 'product_id');
        $combIds = $this->duplicateFieldValueFromList($productsExt['object']['product_options_inventory'], 'combination_hash');
        $prdCombIds = array_merge($productIds, $combIds);
        $prd_comb_ids = $this->arrayToInCondition($prdCombIds);
        $ext_rel_query = array(
            'images_links' => "SELECT i.*, il.* FROM _DBPRF_images_links as il LEFT JOIN _DBPRF_images as i ON i.image_id = il.detailed_id WHERE il.object_type IN ('product', 'product_option') AND il.object_id IN {$prd_comb_ids}",
    ...
    public function convertProduct($product, $productsExt) {
        ...
        if (!$children_product) {
            ...
            $config_data = $this->_importChildrenProduct($product, $productsExt, $children_product);
            if (!$config_data) {
                return array(
                    'result' => 'warning',
                    'msg' => $this->consoleWarning("Product ID = {$product['product_id']} import failed. Error: Product attribute ccould not create!"),
                );
            }
    ...
    protected function _importChildrenProduct($product, $productsExt, $children_product) {
        $result = false;
        $attrMage = $dataChildes = array();
        ...
        foreach ($children_product as $child) {
            $combination = explode('_', $child['combination']);
            $attrChild = array();
            $i = 0;
            while ($i < count($combination)) {
                $option_id = $combination[$i];
                $option_desc = $this->getListFromListByField($productsExt['object']['product_options_descriptions'], 'option_id', $option_id);
                $i++;
                $value_id = $combination[$i];
                $value_desc = $this->getListFromListByField($productsExt['object']['product_option_variants'], 'variant_id', $value_id);
                //Process
                $attr_import = $this->_makeAttributeOptionImport($option_desc, $value_desc, $entity_type_id, $attribute_set_id, $store_view);
                $attr_after = $this->_process->attribute($attr_import['config'], $attr_import['edit']);
                
                $attrMage[$attr_after['attribute_id']]['attribute_label'] = $this->getRowValueFromListByField($option_desc, 'lang_code', $this->_notice['config']['default_lang'], 'option_name');
                ...
                $attrChild[$attr_after['attribute_id']] = $attr_after['option_ids']['option_0'];
                $i++;
            }
            $product_data_add = array(
                'visibility' => Mage_Catalog_Model_Product_Visibility::VISIBILITY_NOT_VISIBLE,
            );
            $product_data = array_merge($this->_convertProduct($product, $productsExt, $child), $product_data_add);
            $pro_import = $this->_process->product($product_data);
            if ($pro_import['result'] !== 'success') {
                var_dump($child['combination_hash']);
                return false;
            }
            foreach ($attrChild as $id => $value) {
                $dataTMP['attribute_id'] = $id;
                $dataTMP['value_index'] = $value;
     ...
     * @return array
     */
    public function _convertProduct($product, $productsExt, $combination = null) {
        $pro_data = array();
        // Attribute
        ...
        if ($attr_pro) {
            ...
            foreach ($attr_pro as $row) {
                $type_src = $this->getRowValueFromListByField($productsExt['object']['product_features'], 'feature_id', $row['feature_id'], 'feature_type');
                $type = $this->_getAttrTypeByTypeSrc($type_src);
                ...
                $attr_import = $this->_makeAttributeImport($attr, $attr_val, $entity_type_id, $attribute_set_id, $store_view, $type);
                $attr_after = $this->_process->attribute($attr_import['config'], $attr_import['edit']);
                if (!$attr_after) {
                    return array(
                        'result' => "warning",
                ...
                if ($type == 'multiselect') {
                    $pro_data[$attr_after['attribute_code']][] = $attr_after['option_ids']['option_0'];
                } elseif ($type == 'select') {
                    $pro_data[$attr_after['attribute_code']] = $attr_after['option_ids']['option_0'];
                } else {
                    $pro_data[$attr_after['attribute_code']] = $attr_val;
                }
            }
        }
        //end Attr
        ...
        if ($product['product_code'] && $combination === null) {
            $pro_data['sku'] = $this->createProductSku($product['product_code'], $this->_notice['config']['languages']);
        } elseif ($combination) {
            if ($combination['product_code']) {
                $pro_data['sku'] = $this->createProductSku($combination['product_code'], $this->_notice['config']['languages']);
            } else {
                $pro_data['sku'] = $this->createProductSku($combination['combination_hash'], $this->_notice['config']['languages']);
            }
        } else {
            $pro_data['sku'] = $this->createProductSku($product['product_id'], $this->_notice['config']['languages']);
        }
        ...
        //Tag
        $productTags = $this->getListFromListByField($productsExt['object']['tag_links'], 'product_id', $product['product_id']);
        $product_tags = $this->duplicateFieldValueFromList($productTags, 'tag');
        $pro_data['tags'] = $product_tags;//$pro_desc_def['search_words'];
        ...
        $price_list = $this->getListFromListByField($price_all, 'percentage_discount', '0');
        $price_final = $this->getRowValueFromListByField($price_list, 'lower_limit', '1', 'price');
        ...
        if ($proTier) {
            foreach ($proTier as $row) {
                if ($row['percentage_discount'] == '0') {
                    ...
                    $price_tier = $row['price'] - ($row['price'] * $row['percentage_discount'] / 100);
                }
                $value = array(
                    'website_id' => 0,
        ...
        $amount = $combination ? $combination['amount'] : $product['amount'];
        $pro_data['stock_data'] = array(
            'is_in_stock' => 1,
            'manage_stock' => 1,//($this->_notice['config']['add_option']['stock'] && $product['amount'] < 1) ? 0 : 1,
            'use_config_manage_stock' => 1,// ($this->_notice['config']['add_option']['stock'] && $product['amount'] < 1) ? 0 : 1,
            'qty' => $amount
        );
        $img_src_id = $combination ? $combination['combination_hash'] : $product['product_id'];
        $proImg = $this->getListFromListByField($productsExt['object']['images_links'], 'object_id', $img_src_id);
        if ($proImg) {
            ...
            if ($image_main) {
                ...
                if ($image_pro && $image_path = $this->downloadImage($this->getUrlSuffix($this->_notice['config']['image_product']), $image_pro, 'catalog/product', false, true)) {
    ...
    public function afterSaveProduct($product_mage_id, $data, $product, $productsExt) {
        ...
        if ($data['type_id'] == 'configurable') return;
    ...
    protected function _getChildrenProduct($product, $productsExt) {
        $child_products = $this->getListFromListByField($productsExt['object']['product_options_inventory'], 'product_id', $product['product_id']);
        if (!$child_products) {
            return false;
        }
    ...
    public function afterSaveCustomer($customer_mage_id, $data, $customer, $customersExt) {
        ...
        if ($cusAdd) {
            ...
            foreach ($signal as $sign) {
                ...
                $address['telephone'] = $cusAdd[$sign . 'phone'] ? $cusAdd[$sign . 'phone'] : $customer['phone'];
    ...
    public function convertOrder($order, $ordersExt) {
        ...
        $order_data['shipping_description'] = isset($shipping_zone[0]['shipping']) ? $shipping_zone[0]['shipping'] : $shipping_zone[1]['shipping'];
        if ($tax_zone) {
            $tax_sum = 0;
...
############################################################ Extend function ##################################
    ...
    protected function _makeAttributeOptionImport($option_desc, $value_desc, $entity_type_id, $attribute_set_id, $store_view) {
        $attr_des = array();
        $attr_des[] = $this->getRowValueFromListByField($option_desc, 'lang_code', $this->_notice['config']['default_lang'], 'option_name');
        $attr_name = $this->joinTextToKey($attr_des[0], 30, '_');
        $opt_des = array();
        $opt_des[] = $this->getRowValueFromListByField($value_desc, 'lang_code', $this->_notice['config']['default_lang'], 'variant_name');
        foreach ($this->_notice['config']['languages'] as $lang_id => $store_id) {
            if ($lang_id == $this->_notice['config']['default_lang']) {
                continue;
            }
            $opt_des[$store_id] = $this->getRowValueFromListByField($value_desc, 'lang_code', $lang_id, 'variant_name');
            $attr_des[$store_id] = $this->getRowValueFromListByField($option_desc, 'lang_code', $lang_id, 'option_name');
        }
        $config = array(
    ...
    protected function _makeAttributeImport($attribute, $option, $entity_type_id, $attribute_set_id, $store_view, $type) {
        $attr_des = array();
    ...
    protected function _getAttrTypeByTypeSrc($type_name) {
        $types = array(
            'M' => 'multiselect',
            'S' => 'select',
            'N' => 'select',
