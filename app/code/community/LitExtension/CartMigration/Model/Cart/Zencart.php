...
class LitExtension_CartMigration_Model_Cart_Zencart
    ...
    public function displayConfig(){
        ...
        $data = $this->_getDataImport($this->_getUrlConnector('query'), array(
            ...
            "query" => serialize(array(
                ...
                "group_pricing" => "SELECT * FROM _DBPRF_group_pricing"
            ))
        ));
        if(!$data || $data['result'] != 'success'){
            return $this->errorConnector();
        ...
        foreach($obj['group_pricing'] as $group_pricing_row){
            $group_pricing_id = $group_pricing_row['group_id'];
            $group_pricing_name = $group_pricing_row['group_name'];
    ...
    protected function _getCategoriesExtQuery($categories){
        ...
        $ext_query = array(
            ...
            'meta_tags_categories_description' => "SELECT * FROM _DBPRF_meta_tags_categories_description WHERE categories_id IN {$cat_id_con}"
        );
        return $ext_query;
    }
    ...
    public function convertCategory($category, $categoriesExt){
        ...
        $cat_desc_def = $this->getRowFromListByField($catDesc, 'language_id', $this->_notice['config']['default_lang']);
        $cat_data['name'] = $cat_desc_def['categories_name'] ? $cat_desc_def['categories_name'] : " ";
        $cat_data['description'] = $cat_desc_def['categories_description'];
        $metaDesc = $this->getListFromListByField($categoriesExt['object']['meta_tags_categories_description'], 'categories_id', $category['categories_id']);
        $meta_desc_def = $this->getRowFromListByField($metaDesc, 'language_id', $this->_notice['config']['default_lang']);
        if($meta_desc_def){
            $cat_data['meta_title'] = $meta_desc_def['metatags_title'];
            $cat_data['meta_keywords'] = $meta_desc_def['metatags_keywords'];
            $cat_data['meta_description'] = $meta_desc_def['metatags_description'];
        }
        ...
        foreach($this->_notice['config']['languages'] as $lang_id => $store_id){
            $store_data = array();
            $cat_desc = $this->getRowFromListByField($catDesc, 'language_id', $lang_id);
            if($lang_id != $this->_notice['config']['default_lang'] && $cat_desc){
                $store_data['store_id'] = $store_id;
                $store_data['name'] = $cat_desc['categories_name'];
                $store_data['description'] = $cat_desc['categories_description'];
                $meta_desc = $this->getRowFromListByField($metaDesc, 'language_id', $lang_id);
                if($meta_desc){
                    $store_data['meta_title'] = $meta_desc['metatags_title'];
                    $store_data['meta_keywords'] = $meta_desc['metatags_keywords'];
                    $store_data['meta_description'] = $meta_desc['metatags_description'];
                }
    ...
    public function prepareImportProducts(){
        ...
        $data = array(
            ...
            'attribute_code'                => 'is_nikelfree',
            'is_global'                     => Mage_Catalog_Model_Resource_Eav_Attribute::SCOPE_STORE,
            'frontend_input'                => 'boolean',
            'frontend_label'                => array('Nikel Free'),
            'option'                        => array(
                'value' => array(
                )
            )
        );
        $this->_process->attribute($data);
    }
    ...
    protected function _getProductsExtQuery($products){
        ...
        $ext_query = array(
            ...
            'products_discount_quantity' => "SELECT * FROM _DBPRF_products_discount_quantity WHERE products_id IN {$pro_ids_query} ORDER BY discount_id ASC",
            'meta_tags_products_description' => "SELECT * FROM _DBPRF_meta_tags_products_description WHERE products_id IN {$pro_ids_query}",
            'tag_to_product' => "SELECT * FROM _DBPRF_tag_to_product AS tp LEFT JOIN _DBPRF_tags AS t ON tp.tag_id = t.tag_id WHERE tp.products_id IN {$pro_ids_query}"
        );
        return $ext_query;
    }
    ...
    public function convertProduct($product, $productsExt){
        ...
        $proAttrSrc = $this->getListFromListByField($productsExt['object']['products_attributes'], 'products_id', $product['products_id']);
        if($proAttrSrc){
            $entity_type_id = Mage::getModel('eav/entity')->setType(Mage_Catalog_Model_Product::ENTITY)->getTypeId();
            $proOptId = $this->duplicateFieldValueFromList($proAttrSrc, 'options_id');
            foreach($proOptId as $pro_opt_id) {
                $proOpt = $this->getListFromListByField($productsExt['object']['products_options'], 'products_options_id', $pro_opt_id);
                $proOptVal = $this->getListFromListByField($proAttrSrc, 'options_id', $pro_opt_id);
                if(!$proOpt || !$proOptVal){
                    continue ;
                }
                $pro_opt_def = $this->getRowFromListByField($proOpt, 'language_id', $this->_notice['config']['default_lang']);
                if ($pro_opt_def['products_options_type'] == 5) {
                    $attr_code = $this->joinTextToKey($pro_opt_def['products_options_name'], 27, '_');
                    $attr_import = array(
                        'entity_type_id' => $entity_type_id,
                        'attribute_code' => 'le_' . $attr_code,
                        'attribute_set_id' => $this->_notice['config']['attribute_set_id'],
                        'frontend_input' => 'multiselect',
                        'frontend_label' => array($pro_opt_def['products_options_name']),
                    ...
                    $optionValues = $optValueToImport = array();
                    foreach($proOptVal as $pro_opt_val){
                        ...
                        $pro_val = $this->getRowFromListByField($proVal, 'language_id', $this->_notice['config']['default_lang']);
                        $option_values = $pro_val['products_options_values_name'];
                        $optionValues[] = $option_values;
                    }
                    if($optionValues){
                        foreach($optionValues as $key => $opt_value){
                            $optValueToImport['option_' . $key] = array($opt_value);
                        }
                    }
                    $attr_import['option']['value'] = $optValueToImport;
                    $attrAfterImport = $this->_process->attribute($attr_import);
                    if($attrAfterImport){
                        $pro_data[$attrAfterImport['attribute_code']] = $attrAfterImport['option_ids'];
                    }
                }else{
                    continue;
                }
        ...
        if(self::CONFIGURABLE_PRODUCT){
            if($proAttrSrc){
                ...
                $config_data = $this->_importChildrenProduct($product, $productsExt, $proAttrSrc);
    ...
    public function afterSaveProduct($product_mage_id, $data, $product, $productsExt){
        ...
        if($product['nation_flag']){
            $entity_type_id = Mage::getModel('eav/entity')->setType(Mage_Catalog_Model_Product::ENTITY)->getTypeId();
            $opt_attr_data = array(
                'entity_type_id'                => $entity_type_id,
                'attribute_set_id'              => $this->_notice['config']['attribute_set_id'],
                'attribute_code'                => 'nation_flag',
                'is_global'                     => Mage_Catalog_Model_Resource_Eav_Attribute::SCOPE_GLOBAL,
                'frontend_input'                => 'select',
                'frontend_label'                => array('National'),
                'option'                        => array(
                    'value' => array(
                        'option_0' => array($product['nation_flag'])
                    )
                )
            );
            $optAttrDataImport = $this->_process->attribute($opt_attr_data);
            if($optAttrDataImport){
                $this->setProAttrSelect($entity_type_id, $optAttrDataImport['attribute_id'], $product_mage_id, $optAttrDataImport['option_ids']['option_0']);
            }
        }
        ...
        if($proAttr && !self::CONFIGURABLE_PRODUCT){
            $opt_data = array();
            ...
            foreach($proOptId as $pro_opt_id){
                ...
                $optType = $this->_createOptionTypeAttribute($pro_opt_def['products_options_type']);
                $option = array(
                    'previous_group' => $optType['previous_group'],
                    'type' => $optType['type'],
                    'is_require' => 1,
                    'title' => $pro_opt_def['products_options_name'],
                    'sort_order' => $pro_opt_def['products_options_sort_order']
                );
                $values = array();
                foreach($proOptVal as $pro_opt_val){
                    $proVal = $this->getListFromListByField($productsExt['object']['products_options_values'], 'products_options_values_id', $pro_opt_val['options_values_id']);
                    $pro_val = $this->getRowFromListByField($proVal, 'language_id', $this->_notice['config']['default_lang']);
                    $pro_attr_val = $this->getRowFromListByField($proAttr, 'options_values_id', $pro_val['products_options_values_id']);
                    $opt_value_price = $pro_attr_val['options_values_price'];
                    if($proSpecial && $proSpecial['status'] == 1 && $proSpecial['specials_new_products_price'] > 0){
                        if($product['products_price'] > 0 && $pro_attr_val['options_values_price']){
                            $opt_value_price = $pro_attr_val['options_values_price'] - ($pro_attr_val['options_values_price'] * (($product['products_price'] - $proSpecial['specials_new_products_price']) / $product['products_price']));
                        }else{
                            $opt_value_price = 0;
                        }
                    }
                    if($optType['previous_group'] != Mage_Catalog_Model_Product_Option::OPTION_GROUP_SELECT){
                        $option['price'] = $pro_attr_val['price_prefix'] . round($opt_value_price,1);
                        $option['price_type'] = 'fixed';
                        break;
                    }
                    $value = array(
                        'option_type_id' => -1,
                        'title' => $pro_val['products_options_values_name'],
                        'price' => $pro_attr_val['price_prefix'] . round($opt_value_price,1),
                        'price_type' => 'fixed',
                        'sort_order' => $pro_attr_val['products_options_sort_order'],
                    );
                    $values[] = $value;
                }
                if($optType['previous_group'] == Mage_Catalog_Model_Product_Option::OPTION_GROUP_SELECT){
    ...
    public function convertCustomer($customer, $customersExt){
        ...
        $cus_data['created_at'] = ($info)? $info['customers_info_date_account_created'] : '';
        $cus_data['is_subscribed'] = $customer['customers_newsletter'];
        $customers_group_pricing = $customer['customers_group_pricing'];
        $cus_data['group_id'] = isset($this->_notice['config']['customer_group'][$customers_group_pricing]) ? $this->_notice['config']['customer_group'][$customers_group_pricing] : 1;
    ...
    public function convertOrder($order, $ordersExt){
        ...
        $ot_group_pricing = $this->getRowValueFromListByField($orderTotal, 'class', 'ot_group_pricing', 'value');
        $discount_amount = 0;
        if($ot_coupon){
            $discount_amount += $ot_coupon;
        }
        if($ot_group_pricing){
            $discount_amount += $ot_group_pricing;
        }
        ...
        if($this->_notice['config']['order_status']){
            if(isset($this->_notice['config']['order_status'][$order_status_id])){
                $order_data['status'] = $this->_notice['config']['order_status'][$order_status_id];
            }else{
                $order_data['status'] = reset($this->_notice['config']['order_status']);
            }
    ...
    protected function _getReviewsMainQuery(){
        ...
        $query = "SELECT r.*, rd.languages_id, rd.reviews_text FROM _DBPRF_reviews AS r LEFT JOIN _DBPRF_reviews_description AS rd ON rd.reviews_id = r.reviews_id WHERE r.reviews_id > {$id_src} ORDER BY r.reviews_id ASC LIMIT {$limit}";
        return $query;
    }
...
############################################################ Extend function ##################################
    ...
    /**
     * Create product tier price by product data and products_discount_quantity table
     */
    protected function _createProductTierPrice($product, $productsExt){
        $result = false;
        $product_id = $product['products_id'];
        $discount_type = $product['products_discount_type'];
        $price = $product['products_price'];
        if($discount_type == 0 || !$price){
            return $result;
        }
        $list = $this->getListFromListByField($productsExt['object']['products_discount_quantity'], 'products_id', $product_id);
        if(!$list){
            return false;
        }
        $result = array();
        foreach($list as $row){
            if($discount_type == 1){
                $tier_price = $price * (100 - $row['discount_price']) /100;
            } else if($discount_type == 2){
                $tier_price = $row['discount_price'] ;
            } else if($discount_type == 3){
                $tier_price = $price  - $row['discount_price'];
            }
            $value = array(
                'website_id' => 0,
                'cust_group' => Mage_Customer_Model_Group::CUST_GROUP_ALL,
                'price_qty' => $row['discount_qty'],
                'price' => $tier_price
            );
            $result[] = $value;
        }
    ...
    protected function _importChildrenProduct($product, $productsExt, $proAttrSrc){
        $proSpecial = $this->getRowFromListByField($productsExt['object']['specials'], 'products_id', $product['products_id']);
        $entity_type_id = Mage::getModel('eav/entity')->setType(Mage_Catalog_Model_Product::ENTITY)->getTypeId();
        $result = $dataChildes = $attrMage = array();
        $attrSrc = array();
        if($proAttrSrc){
            foreach($proAttrSrc as $row){
                $attrSrc[$row['options_id']][] = $row;
            }
        }
        $combAttr = $this->_combinationFromMultiArray($attrSrc);
        if ($combAttr) {
            foreach ($combAttr as $comb_row) {
                $option_collection = '';
                $dataOpts = array();
                $price = $product['products_price'];
                foreach ($comb_row as $pro_attr_src) {
                    $proOptSrc = $this->getListFromListByField($productsExt['object']['products_options'], 'products_options_id', $pro_attr_src['options_id']);
                    $proOptValSrc = $this->getListFromListByField($productsExt['object']['products_options_values'], 'products_options_values_id', $pro_attr_src['options_values_id']);
                    if (!$proOptSrc || !$proOptValSrc) {
                        return array(
                            'result' => "warning",
                            'msg' => $this->consoleWarning("Product Id = {$product['products_id']} import failed. Error: Product attribute could not create!")
                        );
                    }
                    $pro_opt_def = $this->getRowFromListByField($proOptSrc, 'language_id', $this->_notice['config']['default_lang']);
                    $pro_val_def = $this->getRowFromListByField($proOptValSrc, 'language_id', $this->_notice['config']['default_lang']);
                    $attribute_name = $pro_opt_def['products_options_name'];
                    $attribute_code = $this->joinTextToKey($attribute_name, 27, '_');
                    $option_name = $pro_val_def['products_options_values_name'];
                    $opt_attr_data = array(
                        'entity_type_id'                => $entity_type_id,
                        'attribute_set_id'              => $this->_notice['config']['attribute_set_id'],
                    ...
                    //price option
                    $opt_value_price = $pro_attr_src['options_values_price'];
                    if($proSpecial && $proSpecial['status'] == 1 && $proSpecial['specials_new_products_price'] > 0){
                        if($product['products_price'] > 0 && $pro_attr_src['options_values_price']){
                            $opt_value_price = $pro_attr_src['options_values_price'] - ($pro_attr_src['options_values_price'] * (($product['products_price'] - $proSpecial['specials_new_products_price']) / $product['products_price']));
                        }else{
                            $opt_value_price = 0;
                        }
                    }
                    $option_price = $pro_attr_src['price_prefix'] . $opt_value_price;
                    ...
                    $attrMage[$optAttrDataImport['attribute_id']]['values'][$optAttrDataImport['option_ids']['option_0']]['pricing_value'] = $option_price;

                    if($pro_attr_src['price_prefix'] == '+'){
                        $price += $pro_attr_src['options_values_price'];
                    } else {
                        $price -= $pro_attr_src['options_values_price'];
                    }
                }
                ...
                if ($pro_import['result'] !== 'success') {
                    return array(
                        'result' => "warning",
                        'msg' => $this->consoleWarning("Product Id = {$product['products_id']} import failed. Error: Product children could not create!")
                    );
                }
    ...
    protected function _convertProduct($product, $productsExt, $type_id, $is_variation_pro = false, $data_variation = array()){
        ...
        if(!$is_variation_pro){
            if($product['product_is_free'] == 1 || $product['product_is_call'] == 1){
                $pro_data['price'] = 0;
            } else {
                $pro_data['price'] = ($product['products_price']) ? $product['products_price'] : 0;
            }
            $tier_price = $this->_createProductTierPrice($product, $productsExt);
            if($tier_price){
                $pro_data['tier_price'] = $tier_price;
            }
            $proSpecial = $this->getRowFromListByField($productsExt['object']['specials'], 'products_id', $product['products_id']);
            if($proSpecial){
                $pro_data['special_price'] =  $proSpecial['specials_new_products_price'];
                $pro_data['special_from_date'] = $this->_cookSpecialDate($proSpecial['specials_date_available']);
                $pro_data['special_to_date'] = $this->_cookSpecialDate($proSpecial['expires_date']);
            }else{
                $pro_data['special_price'] =  $product['products_price_sorter'];
            }
            ...
            $pro_data['price'] = $data_variation['price'];
        }
        ...
        $metaDesc = $this->getListFromListByField($productsExt['object']['meta_tags_products_description'], 'products_id', $product['products_id']);
        $meta_desc_def = $this->getRowFromListByField($metaDesc, 'language_id', $this->_notice['config']['default_lang']);
        if($meta_desc_def){
            $pro_data['meta_title'] = $meta_desc_def['metatags_title'];
            $pro_data['meta_keyword'] = $meta_desc_def['metatags_keywords'];
            $pro_data['meta_description'] = $meta_desc_def['metatags_description'];
        }
        ...
        if($product['products_image']) {
            $img_extension = substr($product['products_image'], strrpos($product['products_image'], '.'));
            $img_name = preg_replace("/" . $img_extension . "$/", '', $product['products_image']);
            for ($i = 1; $i < 4; $i++) {
                $proPath[] = $img_name . '_0' . $i . $img_extension;
            }
        }
        $img_folder_path = $this->getUrlSuffix($this->_notice['config']['image_product']);
        foreach ($proPath as $pro_path) {
            $url_image = rtrim($img_folder_path, '/') . '/' . $this->_getUrlRealPath($pro_path);
            if($this->_checkUrlImageImport($url_image)){
                if ($gallery_path = $this->downloadImage($this->getUrlSuffix($this->_notice['config']['image_product']), $pro_path, 'catalog/product', false, true)) {
        ...
        foreach($this->_notice['config']['languages'] as $lang_id => $store_id){
            if($lang_id != $this->_notice['config']['default_lang'] && $store_data_change = $this->getRowFromListByField($proDesc, 'language_id', $lang_id)){
                ...
                if($meta_desc){
                    $store_data['meta_title'] = $meta_desc['metatags_title'];
                    $store_data['meta_keyword'] = $meta_desc['metatags_keywords'];
                    $store_data['meta_description'] = $meta_desc['metatags_description'];
                }
                $store_data['store_id'] = $store_id;
                $multi_store[] = $store_data;
        ...
        $pro_data['is_nikelfree'] = $product['is_nikelfree'];
        $tags = array();
        $proTags = $this->getListFromListByField($productsExt['object']['tag_to_product'], 'products_id', $product['products_id']);
        if($proTags){
            foreach($proTags as $pro_tag){
                if($pro_tag['tag_name']){
                    $tags[] = $pro_tag['tag_name'];
                }
            }
        }
        $pro_data['tags'] = $tags;
    ...
    protected function _checkUrlImageImport($url){
    ...
    protected function _createOptionTypeAttribute($type){
        if($type == 1){
            return array(
        ...
        if($type == 4){
            return array(
                'previous_group' => Mage_Catalog_Model_Product_Option::OPTION_GROUP_FILE,
    ...
    public function getAllReviews()
    {
        ...
        $query = "SELECT r.*, rd.languages_id, rd.reviews_text FROM _DBPRF_reviews AS r LEFT JOIN _DBPRF_reviews_description AS rd ON rd.reviews_id = r.reviews_id ORDER BY r.reviews_id ASC";
        $data = $this->_getDataImport($this->_getUrlConnector('query'), array(
            'query' => $query,
        ));
        if(!$data || $data['result'] != 'success'){
