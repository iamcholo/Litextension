...
class LitExtension_CartMigration_Model_Cart_Oxideshop44 extends LitExtension_CartMigration_Model_Cart {
    ...
    public function displayConfig() {
        ...
        if ($this->_notice['extend']['cookie_key']) {
            foreach ($this->_notice['extend']['cookie_key'] as $language) {
    ...
    protected function _getProductsExtQuery($products) {
        ...
        $parentIds = $this->duplicateFieldValueFromList($products['object'], 'OXPARENTID');
        $parentIds_query = $this->arrayToInCondition($parentIds);
        $ext_query = array(
            ...
            'option_of_variant' => "SELECT * FROM _DBPRF_oxarticles WHERE OXID IN {$parentIds_query}"
        );
        return $ext_query;
    }
    ...
    protected function _getProductsExtRelQuery($products, $productsExt) {
        ...
        $ext_rel_query = array(
            ...
            'oxselectlist_all' => "SELECT * FROM _DBPRF_oxselectlist"
        );
...
//      $lang_def = $this->_notice['config']['languages_data'][$this->_notice['config']['default_lang']];
        ...
        if ($product['OXTHUMB'] != '' && $image_path = $this->downloadImage($this->getUrlSuffix($this->_notice['config']['image_product']), '/thumb/' . $product['OXTHUMB'], 'catalog/product', false, true)) {
    ...
    public function afterSaveProduct($product_mage_id, $data, $product, $productsExt) {
        ...
        /** Begin custom code set config to simple */
        
        $field_varname_def = $this->_notice['config']['default_lang'] == 0 ? 'OXVARNAME' : 'OXVARNAME_' . $this->_notice['config']['default_lang'];
        if ($product[$field_varname_def]) {
            $selectslists = explode(' | ', $product[$field_varname_def]);
            $opt_data = array();
            $opt_data_store = array();
            foreach ($selectslists as $key => $select_list) {
                $option = array(
                    'previous_group' => Mage::getModel('catalog/product_option')->getGroupByType('drop_down'),
                    'type' => 'drop_down',
                    'is_require' => 0,
                ...
                foreach ($this->_notice['config']['languages_data'] as $lang_id => $lang_name) {
                    ...
                    $field_varname_lang = $lang_id == 0 ? 'OXVARNAME' : 'OXVARNAME_' . $lang_id;
                    $selectslistslang = explode(' | ', $product[$field_varname_lang]);
                    $option_store[$lang_id] = array(
                        'previous_group' => Mage::getModel('catalog/product_option')->getGroupByType('drop_down'),
                        'type' => 'drop_down',
                        'is_require' => 0,
                        'title' => isset($selectslistslang[$key]) ? $selectslistslang[$key] : ''
                    );
                }
                $values = array();
                $value_stores = array();
                $field_title_def = $this->_notice['config']['default_lang'] == 0 ? 'OXTITLE' : 'OXTITLE_' . $this->_notice['config']['default_lang'];
                $field_valdesc_def = $this->_notice['config']['default_lang'] == 0 ? 'OXVALDESC' : 'OXVALDESC_' . $this->_notice['config']['default_lang'];
                $oxselectlist_row = $this->getRowFromListByField($productsExt['object']['oxselectlist_all'], $field_title_def, $select_list);
                if($oxselectlist_row){
                    $proOptVal = explode('__@@', $oxselectlist_row[$field_valdesc_def]);
                    foreach ($proOptVal as $k => $pro_oppt){
                        if(!$pro_oppt){
                            continue;
                        }
                        ...
                        foreach ($this->_notice['config']['languages_data'] as $lang_id => $lang_name) {
                            ...
                            $option_desc = ($lang_id == 0) ? $oxselectlist_row['OXVALDESC'] : $oxselectlist_row['OXVALDESC_' . $lang_id];
                            $optionAll = explode("__@@", $option_desc);
                            $optionVal = explode("!P!", $optionAll[$k]);
                            $price_ = 0;
                            if(strpos($optionVal[1], '%')){
    ...
    public function importProduct($data, $product, $productsExt){
        ...
        if($productIpt['result'] == 'success'){
            ...
            $this->productSuccess(false, $id_desc, $id_src);
            if ($product_tag) {
