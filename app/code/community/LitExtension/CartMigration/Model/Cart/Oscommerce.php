...
class LitExtension_CartMigration_Model_Cart_Oscommerce
    ...
    public function convertOrder($order, $ordersExt){
        ...
        if($orderStatus){
            ...
            $order_status_id = isset($this->_notice['config']['order_status'][$order_status['orders_status_id']]) ? $this->_notice['config']['order_status'][$order_status['orders_status_id']] : Mage_Sales_Model_Order::STATE_CANCELED;
        }else{
            $order_status_id = Mage_Sales_Model_Order::STATE_CANCELED;
        }
    ...
    protected function _getReviewsMainQuery(){
        ...
        $query = "SELECT r.*, rd.* FROM _DBPRF_reviews AS r LEFT JOIN _DBPRF_reviews_description AS rd ON rd.reviews_id = r.reviews_id WHERE r.reviews_id > {$id_src} ORDER BY r.reviews_id ASC LIMIT {$limit}";
        return $query;
    }
