...
class LitExtension_CartMigration_Model_Cart_Xtcommercev3
    ...
    public function displayConfig(){
        ...
        $data = $this->_getDataImport($this->_getUrlConnector('query'), array(
            ...
            "query" => serialize(array(
                ...
                'customers_status' => "SELECT * FROM _DBPRF_customers_status WHERE language_id = '{$this->_notice['config']['default_lang']}'"
            ))
        ));
        if(!$data || $data['result'] != 'success'){
            return $this->errorConnector();
        ...
        foreach($obj['customers_status'] as $cus_status_row){
    ...
    public function convertCategory($category, $categoriesExt){
        ...
        $cat_default = $this->getRowFromListByField($catDesc, 'language_id', $this->_notice['config']['default_lang']);
        $cat_data['name'] = $cat_default['categories_name'] ? $cat_default['categories_name'] : ' ';
        $cat_data['description'] = $cat_default['categories_description'];
        $cat_data['meta_title'] = $cat_default['categories_meta_title'];
        $cat_data['meta_description'] = $cat_default['categories_meta_description'];
        $cat_data['meta_keywords'] = $cat_default['categories_meta_keywords'];
        if($category['categories_image'] && $img_path = $this->downloadImage($this->getUrlSuffix($this->_notice['config']['image_category']), 'categories/'.$category['categories_image'], 'catalog/category')){
        ...
        foreach($this->_notice['config']['languages'] as $lang_id => $store_id){
            $store_data = array();
            $store_change = $this->getRowFromListByField($catDesc, 'language_id', $lang_id);
            if($lang_id != $this->_notice['config']['default_lang'] && $store_change){
                $store_data['store_id'] = $store_id;
                $store_data['name'] = $store_change['categories_name'];
                $store_data['description'] = $store_change['categories_description'];
                $store_data['meta_title'] = $store_change['categories_meta_title'];
                $store_data['meta_description'] = $store_change['categories_meta_description'];
                $store_data['meta_keywords'] = $store_change['categories_meta_keywords'];
    ...
    protected function _getProductsExtQuery($products){
        ...
        $ext_query = array(
            ...
            'tier_prices_guest' => "SELECT * FROM _DBPRF_personal_offers_by_customers_status_1 WHERE products_id IN {$pro_ids_query} AND personal_offer != 0",
            'tier_prices_new_customer' => "SELECT * FROM _DBPRF_personal_offers_by_customers_status_2 WHERE products_id IN {$pro_ids_query} AND personal_offer != 0",
            'tier_prices_merchant' => "SELECT * FROM _DBPRF_personal_offers_by_customers_status_3 WHERE products_id IN {$pro_ids_query} AND personal_offer != 0",
    ...
    public function convertProduct($product, $productsExt){
        ...
        $pro_data['meta_title'] = $pro_desc_def['products_meta_title'];
        ...
        $tierPrices = array();
        $tier_guest = $this->getListFromListByField($productsExt['object']['tier_prices_guest'], 'products_id', $product['products_id']);
        $tier_new_customer = $this->getListFromListByField($productsExt['object']['tier_prices_new_customer'], 'products_id', $product['products_id']);
        $tier_merchant = $this->getListFromListByField($productsExt['object']['tier_prices_merchant'], 'products_id', $product['products_id']);
        if($tier_guest) $tierPrices = array_merge($tierPrices,$this->_createGrandPriceProduct($tier_guest, 0));
        if($tier_new_customer) $tierPrices = array_merge($tierPrices,$this->_createGrandPriceProduct($tier_new_customer, 1));
        if($tier_merchant) $tierPrices = array_merge($tierPrices,$this->_createGrandPriceProduct($tier_merchant, 2));
        ...
        if($proImg){
            foreach($proImg as $gallery){
                if($gallery_path = $this->downloadImage($this->getUrlSuffix($this->_notice['config']['image_product']), $gallery['image_name'], 'catalog/product', false, true)){
        ...
        foreach($this->_notice['config']['languages'] as $lang_id => $store_id){
            if($lang_id != $this->_notice['config']['default_lang'] && $store_data_change = $this->getRowFromListByField($proDesc, 'language_id', $lang_id)){
                ...
                $store_data['short_description'] = $this->changeImgSrcInText($store_data_change['products_short_description'], $this->_notice['config']['add_option']['img_des']);
    ...
    public function afterSaveProduct($product_mage_id, $data, $product, $productsExt){
            ...
            foreach($proOptId as $pro_opt_id){
                ...
                foreach($proOptVal as $pro_opt_val){
                    ...
                    $value = array(
                        ...
                        'sku'   => $pro_attr_val['attributes_model'],
                        'price' => $pro_attr_val['price_prefix'] . $pro_attr_val['options_values_price'],
                        'price_type' => 'fixed',
                    );
                    $values[] = $value;
                }
        ...
        $this->_importCrossSellProduct($product_mage_id, $product, $productsExt);
    }
        ...
        if($cusAdd){
            foreach($cusAdd as $cus_add){
                ...
                if($cus_add['entry_zone_id'] != 0){
                    $region_id = $this->_getRegionIdByCode($cus_add['zone_code'], $cus_add['countries_iso_code_2']);
                    if($region_id){
    ...
    public function convertOrder($order, $ordersExt){
        ...
        $order_data['customer_firstname'] = $order['customers_firstname'];
        $order_data['customer_lastname'] = $order['customers_lastname'];
    ...
    protected function _getReviewsMainQuery(){
        ...
        $query = "SELECT r.*, rd.*
                                FROM _DBPRF_reviews AS r
                                    LEFT JOIN _DBPRF_reviews_description AS rd ON rd.reviews_id = r.reviews_id
                                WHERE r.reviews_id > {$id_src} ORDER BY r.reviews_id ASC LIMIT {$limit}";
        return $query;
    }
...
############################################################ Extend function ##################################
    ...
    /**
     * Create data grand prices for product
     */
    protected function _createGrandPriceProduct($grandPrices, $mage_group_customer){
        ...
        foreach($grandPrices as $customer_price){
            $tierPrices[] = array(
                ...
                'price_qty'   => $customer_price['quantity'],
                'price'       => $customer_price['personal_offer']
            );
        }
        return $tierPrices;
    }
    ...
    /**
     * Assign Cross-sell Product
     */
    protected function _importCrossSellProduct($product_mage_id, $product, $productsExt){
        $products_links = Mage::getModel('catalog/product_link_api');
        $proCross = $this->getListFromListByField($productsExt['object']['products_xsell'], 'products_id', $product['products_id']);
        if($proCross){
            foreach($proCross as $pro_cross){
                if($pro_id_cross = $this->getMageIdProduct($pro_cross['xsell_id'])){
                    $cross_data = array('position' => $pro_cross['sort_order']);
                    $products_links->assign("cross_sell", $product_mage_id, $pro_id_cross, $cross_data);
                    $products_links->assign("cross_sell", $pro_id_cross, $product_mage_id);
                }else{
                    continue;
                }
            }
        ...
        $proSrc = $this->getListFromListByField($productsExt['object']['products_xsell'], 'xsell_id', $product['products_id']);
        if($proSrc){
            foreach($proSrc as $pro_src){
                if($proSrcId = $this->getMageIdProduct($pro_src['products_id'])){
                    $cross_data = array('position' => $pro_src['sort_order']);
                    $products_links->assign("cross_sell", $proSrcId, $product_mage_id, $cross_data);
                    $products_links->assign("cross_sell", $product_mage_id, $proSrcId);
                }else{
                    continue;
                }
            }
    ...
    public function getAllReviews()
    {
        ...
        $query = "SELECT r.*, rd.*
                                FROM _DBPRF_reviews AS r
                                    LEFT JOIN _DBPRF_reviews_description AS rd ON rd.reviews_id = r.reviews_id ORDER BY r.reviews_id ASC";
        $data = $this->_getDataImport($this->_getUrlConnector('query'), array(
            'query' => $query,
        ));
        if(!$data || $data['result'] != 'success'){
