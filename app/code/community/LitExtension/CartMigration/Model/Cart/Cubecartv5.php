 ...
 */
class LitExtension_CartMigration_Model_Cart_Cubecartv5
    extends LitExtension_CartMigration_Model_Cart
{
    ...
    public function checkRecent()
    {
        $data = $this->_getDataImport($this->_getUrlConnector('query'), array(
            ...
            'query' => serialize(array(
                ...
                'orders' => "SELECT COUNT(1) FROM _DBPRF_order_summary WHERE id > {$this->_notice['orders']['id_src']}",
    ...
    public function displayConfig(){
        ...
        if ($object && $object['config']) {
            ...
            $this->_notice['config']['default_lang'] = isset($config_data['default_language']) ? $config_data['default_language'] : 'en-US';
    ...
    protected function _getTaxesExtQuery($taxes){
        ...
        $ext_query = array(
            'tax_rates' => "SELECT tr.*, gc.*, gz.abbrev, gz.name as zone_name
                            FROM _DBPRF_tax_rates AS tr
                              LEFT JOIN _DBPRF_geo_country AS gc ON gc.id = tr.country_id
   ...
   /* protected function _getTaxesExtRelQuery($taxes, $taxesExt)
    {
        ...
        $tax_zone_query = $this->duplicateFieldValueFromList($taxesExt['object']['tax_rates'], 'tax_zone_id');
        $ext_rel_query = array(
            'zones_to_geo_zones' => "SELECT ztgz.*, z.*, c.*
												FROM _DBPRF_zones_to_geo_zones AS ztgz
													LEFT JOIN _DBPRF_zones_to_geo_zones AS ztgz
    ...
    */
    ...
    public function convertCategory($category, $categoriesExt){
        ...
        //$cat_def = $this->getRowFromListByField($catDesc, 'language', $this->_notice['config']['default_lang']);
    ...
    protected function _getProductsExtQuery($products){
        ...
        $ext_query = array(
			...
			'category_index' => "SELECT * _DBPRF_category_index WHERE product_id IN {$pro_ids_query}",
    ...
    public function convertProduct($product, $productsExt){
        ...
        $pro_data['description'] = $product['description'];
      ...
      //  }
       ...
       // if ($product['tax_class_id'] != 0 && $tax_pro_id = $this->getMageIdTaxProduct($product['tax_type'])) {
       ...
       // }
        ...
        foreach ($this->_notice['config']['languages'] as $lang_id => $store_id) {
            if ($lang_id != $this->_notice['config']['default_lang'] && $store_data_change = $this->getRowFromListByField($proDesc, 'language', $lang_id)) {
				...
				$store_data['meta_keyword'] = $store_data_change['seo_meta_keyword'];
				$store_data['meta_tdescription'] = $store_data_change['seo_meta_description'];
                $multi_store[] = $store_data;
            }
        }
        ...
        if ($proAttr) {
            ...
            foreach ($proOptId as $pro_opt_id) {
                ...
                foreach ($proOptVal as $pro_opt_val) {
                    $proVal = $this->getListFromListByField($productsExt['object']['option_value'], 'value_id', $pro_opt_val['value_id']);
                    $value = array(
                        'option_type_id' => -1,
    ...
    protected function _getCustomersExtQuery($customers){
        ...
        $ext_query = array(
            'address_book' => "SELECT ab.*, gc.* FROM _DBPRF_addressbook AS ab
                                  LEFT JOIN _DBPRF_geo_country AS gc ON gc.id = ab.country
                               WHERE ab.customer_id IN {$customer_ids_query}",
    ...
    public function afterSaveCustomer($customer_mage_id, $data, $customer, $customersExt){
        ...
        if ($cusAdd) {
            foreach ($cusAdd as $cus_add) {
                ...
                $address['country_id'] = $cus_add['iso'];
                ...
                $address['fax'] = '';
                if ($cus_add['state']) {
                    $region_id = $this->getRegionId($cus_add['state'], $cus_add['iso']);
                    if ($region_id) {
    ...
    protected function _getOrdersMainQuery(){
        ...
        $query = "SELECT * FROM _DBPRF_order_summary WHERE cart_order_id > {$id_src} ORDER BY cart_order_id ASC LIMIT {$limit}";
        return $query;
    }
    ...
    public function convertOrder($order, $ordersExt){
        ...
        $orderPro = $this->getListFromListByField($ordersExt['object']['order_inventory'], 'cart_orders_id', $order['cart_orders_id']);
       // $orderProOpt = $this->getListFromListByField($ordersExt['object']['orders_products_attributes'], 'orders_id', $order['orders_id']);
        ...
        $orderStatus = $this->getListFromListByField($ordersExt['object']['orders_history'], 'cart_order_id', $order['cart_order_id']);
		...
		$order_tax_amount = $this->getRowValueFromListByField($ordersExt['object']['order_tax'], 'cart_order_id', $order['cart_order_id'], 'amount');
        if ($order_tax_amount) {
        ...
        $data['carts'] = $carts;
        $data['order_src_id'] = $order['cart_orders_id'];
    ...
    public function afterSaveOrder($order_mage_id, $data, $order, $ordersExt){
		...
		$orderNotes = $this->getListFromListByField($ordersExt['object']['order_notes'], 'cart_order_id', $order['cart_oder_id']);
    ...
    public function convertReview($review, $reviewsExt){
        ...
        if (!$product_mage_id) {
            return array(
                'result' => 'warning',
                'msg' => $this->consoleWarning("Review Id = {$review['id']} import failed. Error: Product Id = {$review['products_id']} not imported!")
            );
        }
		$store_id = $this->_notice['config']['languages'][$this->_notice['config']['default_lang']];
        $data = array();
...
############################################################ Extend function ##################################
    ...
    public function getAllOrders()
    {
        ...
        $query = "SELECT * FROM _DBPRF_order_summary ORDER BY cart_order_id ASC";
        $data = $this->_getDataImport($this->_getUrlConnector('query'), array(
            'query' => $query,
        ));
        if(!$data || $data['result'] != 'success'){
