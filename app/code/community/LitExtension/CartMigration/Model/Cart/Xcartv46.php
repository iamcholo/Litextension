...
class LitExtension_CartMigration_Model_Cart_Xcartv46
    ...
    public function checkRecent()
    {
        $data = $this->_getDataImport($this->_getUrlConnector('query'), array(
            ...
            'query' => serialize(array(
                ...
                'customers' => "SELECT COUNT(1) FROM _DBPRF_customers WHERE id > {$this->_notice['customers']['id_src']}",
    ...
    public function convertCategory($category, $categoriesExt){
        ...
        $cat_data['meta_title'] = $category['title_tag'];
    ...
    public function getProductsExt($products){
        ...
        $ext_query = array(
            ...
            'images_t_gold' => "SELECT * FROM _DBPRF_images_T AS img_pro WHERE img_pro.id IN {$proIds_in_query}",
            'images_d_gold' => "SELECT * FROM _DBPRF_images_D AS img_d WHERE img_d.id IN {$proIds_in_query} ",
            'images_p_gold' => "SELECT * FROM _DBPRF_images_P AS img_d WHERE img_d.id IN {$proIds_in_query} ",
            'images_z_gold' => "SELECT * FROM _DBPRF_images_Z AS img_d WHERE img_d.id IN {$proIds_in_query} ",
            'images_t_gold_plus' => "SELECT * FROM _DBPRF_images_t AS img_pro WHERE img_pro.id IN {$proIds_in_query}",
            'images_d_gold_plus' => "SELECT * FROM _DBPRF_images_d AS img_d WHERE img_d.id IN {$proIds_in_query} ",
            'images_p_gold_plus' => "SELECT * FROM _DBPRF_images_p AS img_d WHERE img_d.id IN {$proIds_in_query} ",
            'images_z_gold_plus' => "SELECT * FROM _DBPRF_images_z AS img_d WHERE img_d.id IN {$proIds_in_query} ",
        ...
        foreach($this->_notice['config']['languages'] as $lng_code => $value){
            $tmp_query = array(
                'product_lng_'.$lng_code => "SELECT * FROM _DBPRF_products_lng_{$lng_code} WHERE productid IN {$proIds_in_query}"
            );
            $ext_query = array_merge($ext_query, $tmp_query);
        }
        if($this->_seo){
        ...
        $ext_rel_query = array(
            ...
            'images_w_gold' => "SELECT * FROM _DBPRF_images_W AS img WHERE img.id IN {$varyIds_in_query}",
            'images_w_gold_plus' => "SELECT * FROM _DBPRF_images_w AS img WHERE img.id IN {$varyIds_in_query}",
        );
        if ($this->_seo) {
    ...
    protected function _getCustomersMainQuery(){
        ...
        $query = "SELECT * FROM _DBPRF_customers WHERE id > {$id_src} ORDER BY id ASC LIMIT {$limit}";
        return $query;
    }
    ...
    protected function _getCustomersExtQuery($customers){
        $cusIds = $this->duplicateFieldValueFromList($customers['object'], 'id');
        $cusIds_in_query = $this->arrayToInCondition($cusIds);
        $ext_query = array(
            'address_book' => "SELECT * FROM _DBPRF_address_book WHERE userid IN {$cusIds_in_query}"
        );
        return $ext_query;
    }
    ...
    public function afterSaveCustomer($customer_mage_id, $data, $customer, $customersExt){
        ...
        $cusAdd = $this->getListFromListByField($customersExt['object']['address_book'], 'userid', $customer['id']);
        if($cusAdd){
            foreach($cusAdd as $cus_add){
                $result = array();
                $result['firstname'] = $cus_add['firstname'];
                $result['lastname'] = $cus_add['lastname'];
                $result['country_id'] = $cus_add['country'];
                $street = preg_split('/\n|\r\n?/',$cus_add['address']);
                $result['street'][0] = isset($street[0]) ? $street[0] : ' ';
                $result['street'][1] = isset($street[1]) ? $street[1] : ' ';
                $result['postcode'] = $cus_add['zipcode'];
                $result['city'] = $cus_add['city'];
                $region_id = $this->_getRegionIdByCode($cus_add['state'],$result['country_id']);
                if($region_id){
                    $result['region_id'] = $region_id;
                }else{
                    $result['region'] = $cus_add['state'];
                }
                $result['telephone'] = $cus_add['phone'];
                $result['fax'] = $cus_add['fax'];
                $default_billing = ($cus_add['default_b'] == 'Y')? 1 : 0;
                $default_shipping = ($cus_add['default_s'] == 'Y')? 1 : 0;
                $customAddress = Mage::getModel('customer/address');
                $customAddress->setData($result)
    ...
    public function convertOrder($order, $ordersExt){
        ...
        $address_billing['telephone'] = $order['b_phone'];
        ...
        $address_shipping['telephone'] = $order['s_phone'];
        ...
        $customer_mage_id = $this->getMageIdCustomer($order['userid']);
        if($customer_mage_id){
        ...
        $order_data['shipping_description'] = $order['shipping'];
    ...
    public function convertReview($review, $reviewsExt){
        ...
        if(isset($review['status'])){
            $data['status_id'] = 3;
            if($review['status'] == 'A') $data['status_id'] = 1;
            if($review['status'] == 'P') $data['status_id'] = 2;
        }else{
            $data['status_id'] = 2;
        }
        $data['title'] = ' ';
        ...
        if(isset($review['userid'])){
            $data['customer_id'] = ($this->getMageIdCustomer($review['userid']))? $this->getMageIdCustomer($review['userid']) : null;
        }else{
            $data['customer_id'] = null;
        }
        if(isset($review['author'])){
            $data['nickname'] = $review['author'] ? $review['author'] : $review['email'];
        }else{
            $data['nickname'] = $review['email'];
        }
        if(isset($review['rating'])){
            $data['rating'] = $review['rating']/20;
        }else{
            $data['rating'] = $review['vote_value']/20;
        }
        if(isset($review['datetime'])){
            $data['created_at'] = date("Y-m-d H:i:s",$review['datetime']);
        }else{
...
####################################### Extend Functions #######################################################
    ...
    protected function _convertProduct($parent_product, $product, $productsExt, $visibility, $type_id, $option_collection, $check_variant_pro = false){
        $pro_data = array();
        $pro_data['type_id'] = $type_id;
        $categories = $tierPrices = array();
        $images_list_p = $images_list_d = $images_list_z = false;
        if($check_variant_pro){
            $default_data = $this->getRowFromListByField($productsExt['object']['product_lng_'.$this->_notice['config']['default_lang']], 'productid', $parent_product['productid']);
            $pro_data['price'] = $product['price'];
            $pro_data['name'] = $default_data['product'] ? $default_data['product'].' '.$option_collection : ' ';
            $thumb_img = $this->getRowFromListByField($productsExt['object']['images_w_gold'], 'id', $product['variantid']);
            if(!$thumb_img) $thumb_img = $this->getRowFromListByField($productsExt['object']['images_w_gold_plus'], 'id', $product['variantid']);
            if($thumb_img  && $image_path = $this->downloadImage($this->getUrlSuffix($this->_notice['config']['image_product']), '/W/'.$thumb_img['filename'], 'catalog/product', false, true)){
                $pro_data['image_import_path'] = array('path' => $image_path, 'label' => $thumb_img['alt'] ? $thumb_img['alt'] : $default_data['product']);
            }
            $pro_data['description'] = $this->changeImgSrcInText($default_data['fulldescr'], $this->_notice['config']['add_option']['img_des']);
            $pro_data['short_description'] = $this->changeImgSrcInText($default_data['descr'], $this->_notice['config']['add_option']['img_des']);
            $pro_data['status'] = ($parent_product['forsale'] == 'N') ? 2 : 1;
        }else{
            $pro_data['status'] = ($product['forsale'] == 'N') ? 2 : 1;
            $default_data = $this->getRowFromListByField($productsExt['object']['product_lng_'.$this->_notice['config']['default_lang']], 'productid', $product['productid']);
            $pro_data['price'] = $product['price'];
            ...
            $pro_data['name'] =  $default_data['product'] ? $default_data['product'] : ' ';
            $thumb_img = $this->getRowFromListByField($productsExt['object']['images_t_gold'], 'id', $product['productid']);
            if(!$thumb_img){
                $thumb_img = $this->getRowFromListByField($productsExt['object']['images_t_gold_plus'], 'id', $product['productid']);
            }
            ...
            $images_list_d = $this->getListFromListByField($productsExt['object']['images_d_gold'], 'id', $product['productid']);
            if(!$images_list_d){
                $images_list_d = $this->getListFromListByField($productsExt['object']['images_d_gold_plus'], 'id', $product['productid']);
            }
            $images_list_p = $this->getListFromListByField($productsExt['object']['images_p_gold'], 'id', $product['productid']);
            if(!$images_list_p){
                $images_list_p = $this->getListFromListByField($productsExt['object']['images_p_gold_plus'], 'id', $product['productid']);
            }
            $images_list_z = $this->getListFromListByField($productsExt['object']['images_z_gold'], 'id', $product['productid']);
            if(!$images_list_z){
                $images_list_z = $this->getListFromListByField($productsExt['object']['images_z_gold_plus'], 'id', $product['productid']);
            }
        ...
        if($images_list_z){
            foreach($images_list_z as $gallery){
                $img_z_path = '/Z/'.$gallery['id'].'/'.$gallery['imageid'].'/init_image.jpg';
                if($gallery_path = $this->downloadImage($this->getUrlSuffix($this->_notice['config']['image_product']), $img_z_path, 'catalog/product', true, true)){
                    $pro_data['image_gallery'][] = array('path' => $gallery_path, 'label' => $gallery['alt'] ? $gallery['alt'] : $pro_data['name']) ;
                }
            }
        }
        ...
        $pro_data['meta_keyword'] = isset($product['meta_keywords']) ? $product['meta_keywords'] : ' ';
        ...
        $multi_store = array();
        foreach($this->_notice['config']['languages'] as $lang_code => $value){
            if($lang_code == $this->_notice['config']['default_lang']) continue ;
            if($check_variant_pro){
                $pro_store = $this->getRowFromListByField($productsExt['object']['product_lng_'.$lang_code], 'productid', $parent_product['productid']);
            }else{
                $pro_store = $this->getRowFromListByField($productsExt['object']['product_lng_'.$lang_code], 'productid', $product['productid']);
            }
            if(!$pro_store) continue ;
            $store_data = array();
            if($pro_store['product']) $store_data['name'] = $pro_store['product'];
            if($pro_store['fulldescr']) $store_data['description'] = $this->changeImgSrcInText($pro_store['fulldescr'],$this->_notice['config']['add_option']['img_des']);
            if($pro_store['descr']) $store_data['short_description'] = $this->changeImgSrcInText($pro_store['descr'],$this->_notice['config']['add_option']['img_des']);
            if(!empty($store_data)){
                $store_data['store_id'] = $value;
                $multi_store[] = $store_data;
            }
            ...
            if ($children['quantity'] == 1 && $children['membershipid'] == '0') {
                ...
                if ($options_attribute) {
                    foreach ($options_attribute as $option) {
                        ...
                        $dataOpts[] = $dataTMP;
                        if ($option['option_name']) $option_collection = $option_collection . ' - ' . $option['option_name'];
                    }
                }
                $convertPro = $this->_convertProduct($parent_product, $children, $productsExt, Mage_Catalog_Model_Product_Visibility::VISIBILITY_NOT_VISIBLE, Mage_Catalog_Model_Product_Type::TYPE_SIMPLE, $option_collection, true);
    ...
    public function getAllCustomers()
    {
        ...
        $query = "SELECT * FROM _DBPRF_customers ORDER BY id ASC";
        $data = $this->_getDataImport($this->_getUrlConnector('query'), array(
            'query' => $query,
        ));
        if(!$data || $data['result'] != 'success'){
