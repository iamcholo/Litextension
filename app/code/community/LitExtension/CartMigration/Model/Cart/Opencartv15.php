 ...
 */
class LitExtension_CartMigration_Model_Cart_Opencartv15 extends LitExtension_CartMigration_Model_Cart {
    ...
    public function convertProduct($product, $productsExt) {
        ...
        if($proTier) {
            foreach ($proTier as $row) {
                $value = array(
                    'website_id' => 0,
                    'cust_group' => isset($this->_notice['config']['customer_group'][$row['customer_group_id']]) ? $this->_notice['config']['customer_group'][$row['customer_group_id']] : Mage_Customer_Model_Group::CUST_GROUP_ALL,
