 ...
 */
class LitExtension_CartMigration_Model_Cart_Woocommercev2
    ...
    public function getProductsExt($products){
        ...
        foreach($productIds as $pro_id){
            $table_cross = 'crosssell_ids_' . $pro_id;
            $like_first = '%:"' . $pro_id . '";%';
            $like_second = '%:' . $pro_id . ';%';
            $ext_query[$table_cross] = "SELECT * FROM _DBPRF_postmeta WHERE meta_key = '_crosssell_ids' AND meta_value LIKE '{$like_first}' OR meta_value LIKE '{$like_second}'";
            $table_upsell = 'upsell_ids' . $pro_id;
            $ext_query[$table_upsell] = "SELECT * FROM _DBPRF_postmeta WHERE meta_key = '_upsell_ids' AND meta_value LIKE '{$like_first}' OR meta_value LIKE '{$like_second}'";
        }
        if($this->_seo){
    ...
    public function convertProduct($product, $productsExt){
        ...
        if($pro_data['type_id'] == Mage_Catalog_Model_Product_Type::TYPE_CONFIGURABLE && $variant_products && isset($variant_products[0]['ID'])){
        ...
        if($parentCross = $productsExt['object'][$table]){
            foreach($parentCross as $parent_cross){
                if($parentCrossIdDesc = $this->getMageIdProduct($parent_cross['post_id'])){
                    $products_links->assign("cross_sell", $parentCrossIdDesc, $product_mage_id);
                }
            }
        }
            ...
            if (is_array($crossSellIds)) {
                foreach($crossSellIds as $cross_sell_id) {
                    if ($pro_cross_id_desc = $this->getMageIdProduct($cross_sell_id)) {
                        $products_links->assign("cross_sell", $product_mage_id, $pro_cross_id_desc);
                    }
                }
            }
        }
        ...
        if($parentUpsell = $productsExt['object'][$table]){
            foreach($parentUpsell as $parent_upsell){
                if($parentUpSellIdDesc = $this->getMageIdProduct($parent_upsell['post_id'])){
                    $products_links->assign("up_sell", $parentUpSellIdDesc, $product_mage_id);
                }
            }
        }
        ...
        if($upSellIds) {
            ...
            if (is_array($upSellIds)) {
                foreach($upSellIds as $up_sell_id) {
                    if ($pro_upSell_id_desc = $this->getMageIdProduct($up_sell_id)) {
                        $products_links->assign("up_sell", $product_mage_id, $pro_upSell_id_desc);
                    }
                }
            }
        }
    ...
    public function convertOrder($order, $ordersExt){
        ...
        if(strpos($order['post_status'], 'wc-')){
    ...
    public function afterSaveOrder($order_mage_id, $data, $order, $ordersExt){
        ...
        if($order_comments){
            ...
            $order_data['status'] = $data['order']['status'];
            if(isset($data['order']['shipping_description'])){
                $order_data['comment'] = "<b>Reference order #".$order['ID']."</b><br /><b>Payment method: </b>".$data['woo_payment_method']."<br /><b>Shipping method: </b> ".$data['order']['shipping_description']."<br /><br />";
            }else{
                $order_data['comment'] = "<b>Reference order #".$order['ID']."</b><br /><b>Payment method: </b>".$data['woo_payment_method']."<br /><br />";
            }
            $order_data['is_customer_notified'] = 0;
            $order_data['updated_at'] = $data['order']['created_at'];
            $order_data['created_at'] = $data['order']['created_at'];
            $this->_process->ordersComment($order_mage_id, $order_data);
        }
    }
...
###################################### Extend Function #################################################################
    ...
    protected function _convertProduct($product, $productsExt, $product_meta){
        ...
        $pro_data['status'] = ($product['post_status'] == 'trash' || $product['post_status'] == 'draft') ? 2 : 1;
    ...
    protected function _importChildrenProductAnyAttribute($parent_product, $sku_parent, $productsExt, $variant_products, $parent_meta, $term_relationship, $tax_parent_id){
        ...
        foreach($variant_products as $child){
            ...
            foreach($options_list as $key => $opt){
                if(!in_array($opt['meta_key'], $meta_key_list)){
            ...
            foreach($combination as $combination_child){
                ...
                //
                $convertPro = $attr_pro_data = array();
                $option_collection = '';
    ...
    protected function _getDataOptionVariantPro($data_option, $allOptionAttributeIn, $allOptionAttributeOut, $woo_attribute_list){
        ...
        if($attributePrefix == 'attribute_pa_'){
            ...
            $result['attr_slug'] = $attr_slug;
            $result['attr_code'] = $this->joinTextToKey(str_replace('-',' ',$result['attr_slug']), 30, '_') . '_pa';
