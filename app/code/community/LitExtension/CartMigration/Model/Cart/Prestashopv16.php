 ...
 */
class LitExtension_CartMigration_Model_Cart_Prestashopv16 extends LitExtension_CartMigration_Model_Cart {
    ...
    public function checkRecent()
    {
        $data = $this->_getDataImport($this->_getUrlConnector('query'), array(
            ...
            'query' => serialize(array(
                'taxes' => "SELECT COUNT(1) FROM _DBPRF_tax_rules_group WHERE id_tax_rules_group > {$this->_notice['taxes']['id_src']}",
                'manufacturers' => "SELECT COUNT(1) FROM _DBPRF_manufacturer WHERE id_manufacturer > {$this->_notice['manufacturers']['id_src']}",
                'categories' => "SELECT COUNT(1) FROM _DBPRF_category WHERE id_category > {$this->_notice['categories']['id_src']} AND id_category != {$this->_notice['extend']['highest_root']} AND id_category != {$this->_notice['extend']['root_category']}",
                ...
                'reviews' => "SELECT COUNT(1) FROM _DBPRF_product_comment WHERE id_product_comment > {$this->_notice['reviews']['id_src']}"
            ))
        ));
    ...
    public function displayConfig() {
        ...
        $default_cfg = $this->_getDataImport($this->_getUrlConnector('query'), array(
            ...
            'query' => serialize(array(
                ...
                "configuration" => "SELECT * FROM _DBPRF_configuration WHERE name = 'PS_ROOT_CATEGORY'",
            ))
        ));
        ...
        if ($object && $object['languages'] && $object['currencies'] && $object['configuration']) {
            ...
            $this->_notice['extend']['highest_root'] = isset($object['configuration']['0']['value']) ? $object['configuration']['0']['value'] : 1;
        }
        $data = $this->_getDataImport($this->_getUrlConnector('query'), array(
            "serialize" => true,
            "query" => serialize(array(
                ...
                "category" => "SELECT *, COUNT(*) as c FROM _DBPRF_category WHERE id_parent = {$this->_notice['extend']['highest_root']}",
        ...
        foreach ($obj['languages'] as $language_row) {
            ...
            $lang_name = $language_row['name'] . "(" . $language_row['language_code'] . ")";
        ...
        if ($obj['category']['0']['c'] == 1) {
            $this->_notice['extend']['root_category'] = $obj['category']['0']['id_category'];
        } else {
            $this->_notice['extend']['root_category'] = $this->_notice['extend']['highest_root'];
        }
    ...
    protected function _getTaxesMainQuery() {
        ...
        $query = "SELECT * FROM _DBPRF_tax_rules_group WHERE id_tax_rules_group > {$id_src} ORDER BY id_tax_rules_group ASC LIMIT {$limit}";
        return $query;
    }
    ...
    protected function _getTaxesExtQuery($taxes) {
        $taxRuleIds = $this->duplicateFieldValueFromList($taxes['object'], 'id_tax_rules_group');
        $tax_rule_id_con = $this->arrayToInCondition($taxRuleIds);
        $ext_query = array(
            'tax_rules' => "SELECT * FROM _DBPRF_tax_rule WHERE id_tax_rules_group IN {$tax_rule_id_con}",
        );
        return $ext_query;
    }
    ...
    protected function _getTaxesExtRelQuery($taxes, $taxesExt) {
        $taxIds = $this->duplicateFieldValueFromList($taxesExt['object']['tax_rules'], 'id_tax');
        $tax_query = $this->arrayToInCondition($taxIds);
        $countries = $this->duplicateFieldValueFromList($taxesExt['object']['tax_rules'], 'id_country');
        $countries_query = $this->arrayToInCondition($countries);
        $states = $this->duplicateFieldValueFromList($taxesExt['object']['tax_rules'], 'id_state');
        $states_query = $this->arrayToInCondition($states);
        $ext_rel_query = array(
            'tax_lang' => "SELECT t.*, tl.* FROM _DBPRF_tax as t LEFT JOIN _DBPRF_tax_lang as tl ON tl.id_tax = t.id_tax WHERE t.id_tax IN {$tax_query}",
            'country' => "SELECT * FROM _DBPRF_country WHERE id_country IN {$countries_query}",
    ...
    public function convertTax($tax, $taxesExt) {
        ...
        if ($tax_pro_ipt['result'] == 'success') {
            ...
            $this->taxProductSuccess($tax['id_tax_rules_group'], $tax_pro_ipt['mage_id']);
        }
        $taxZone = $this->getListFromListByField($taxesExt['object']['tax_rules'], 'id_tax_rules_group', $tax['id_tax_rules_group']);
        foreach ($taxZone as $tax_zone) {
            $tax_lang = $this->getRowFromListByField($taxesExt['object']['tax_lang'], 'id_tax', $tax_zone['id_tax']);
            $country_code = $this->getRowValueFromListByField($taxesExt['object']['country'], 'id_country', $tax_zone['id_country'], 'iso_code');
            if ($tax_zone['id_state'] == 0) {
                $zone = 'All States';
            } else {
                $zone = $this->getRowValueFromListByField($taxesExt['object']['state'], 'id_state', $tax_zone['id_state'], 'iso_code');
                $zone_name = $this->getRowValueFromListByField($taxesExt['object']['state'], 'id_state', $tax_zone['id_state'], 'name');
            }
            if (!$country_code) {
                continue;
            }
            $tax_rate_data = array();
            $code = $country_code . "-" . $zone . "-" . $tax_lang['name'];
            $tax_rate_data['code'] = $this->createTaxRateCode($code);
            $tax_rate_data['tax_country_id'] = $country_code;
            if ($tax_zone['id_state'] == 0) {
                $tax_rate_data['tax_region_id'] = 0;
            } else {
                $tax_rate_data['tax_region_id'] = $this->getRegionId($zone_name, $country_code);
            }
            $tax_rate_data['zip_is_range'] = 0;
            $tax_rate_data['tax_postcode'] = "*";
            $tax_rate_data['rate'] = $tax_lang['rate'];
    ...
    protected function _getCategoriesMainQuery() {
        ...
        $query = "SELECT * FROM _DBPRF_category WHERE id_category > {$id_src} AND id_category != {$this->_notice['extend']['highest_root']} AND id_category != {$this->_notice['extend']['root_category']} ORDER BY id_category ASC LIMIT {$limit}";
        return $query;
    }
    ...
    public function convertCategory($category, $categoriesExt) {
        ...
        if (!$category['id_parent'] || $category['id_parent'] == $this->_notice['extend']['root_category'] || $category['id_parent'] == $category['id_category']) {
    ...
    public function getProductsExt($products) {
        ...
        $ext_query = array(
            ...
            'specific_price' => "SELECT * FROM _DBPRF_specific_price WHERE id_product IN {$pro_ids_query}",
            'category_product' => "SELECT * FROM _DBPRF_category_product WHERE id_product IN {$pro_ids_query}",
            'stock_available' => "SELECT * FROM _DBPRF_stock_available WHERE id_product IN {$pro_ids_query}",
        ...
        $imageIds = $this->duplicateFieldValueFromList($productsExt['object']['image'], 'id_image');
        $imageIds_query = $this->arrayToInCondition($imageIds);
        $ext_rel_query = array(
            'image_lang' => "SELECT * FROM _DBPRF_image_lang WHERE id_image IN {$imageIds_query}",
            ...
            'product_attribute_image' => "SELECT * FROM _DBPRF_product_attribute_image WHERE id_product_attribute IN {$product_attr_ids_query}",
        );
        if ($this->_seo) {
    ...
    public function convertCustomer($customer, $customersExt) {
        ...
        $cus_data['group_id'] = isset($this->_notice['config']['customer_group'][$customer['id_default_group']]) ? $this->_notice['config']['customer_group'][$customer['id_default_group']] : 1;
     ...
     * @return array : Response of connector
     */
    protected function _getOrdersExtQuery($orders) { // why not use getOrdersExtQuery
        ...
        $ext_query = array(
            ...
            'order_history' => "SELECT *  FROM _DBPRF_order_history WHERE id_order IN {$order_ids_query} ORDER BY id_order_history ASC",
            ...
            'customer_thread' => "SELECT * FROM _DBPRF_customer_thread WHERE id_order IN {$order_ids_query}",
        );
        return $ext_query;
    }
    ...
    protected function _getOrdersExtRelQuery($orders, $ordersExt) {
        ...
        $mesIds = $this->duplicateFieldValueFromList($ordersExt['object']['customer_thread'], 'id_customer_thread');
        ...
        $mes_ids_query = $this->arrayToInCondition($mesIds);
        $ext_rel_query = array(
            ...
            'customer_message' => "SELECT * FROM _DBPRF_customer_message WHERE id_customer_thread IN {$mes_ids_query}",
        );
        return $ext_rel_query;
    }
    ...
    public function convertOrder($order, $ordersExt) {
        ...
        foreach ($orderPro as $order_pro) {
            ...
            $cart['sku'] = $sku;
            $cart['price'] = $order_pro['unit_price_tax_excl'];
            $cart['original_price'] = $order_pro['original_product_price'];
            $cart['tax_amount'] = $order_pro['total_price_tax_incl'] - $order_pro['total_price_tax_excl'];
            $cart['tax_percent'] = $order['carrier_tax_rate'];
            $cart['discount_amount'] = '0';
            $cart['qty_ordered'] = $order_pro['product_quantity'];
            $cart['row_total'] = $order_pro['total_price_tax_excl'];
            $carts[] = $cart;
        }
        ...
        if ($this->_notice['config']['order_status']) {
            $order_data['status'] = $this->_notice['config']['order_status'][$order['current_state']];
        ...
        $order_data['shipping_amount'] = $order['total_shipping_tax_incl'];
        $order_data['base_shipping_amount'] = $order['total_shipping_tax_incl'];
        $order_data['base_shipping_invoiced'] = $order['total_shipping_tax_incl'];
        $order_data['shipping_description'] = ($shipping_name) ? $shipping_name : 'Default shipping';
        if ($order['carrier_tax_rate']) {
            $order_data['tax_amount'] = $order['carrier_tax_rate'] * $order_data['subtotal'] / 100;
            $order_data['base_tax_amount'] = $order['carrier_tax_rate'] * $order_data['subtotal'] / 100;
        }
        $order_data['discount_amount'] = $order['total_discounts_tax_incl'];
        $order_data['base_discount_amount'] = $order['total_discounts'];
        $order_data['grand_total'] = $this->incrementPriceToImport($order['total_paid_tax_incl']);
    ...
    public function afterSaveOrder($order_mage_id, $data, $order, $ordersExt) {
        ...
        $orderThread = $this->getRowValueFromListByField($ordersExt['object']['customer_thread'], 'id_order', $order['id_order'], 'id_customer_thread');
        $orderComment = $this->getListFromListByField($ordersExt['object']['customer_message'], 'id_customer_thread', $orderThread);
        $timeline = array();
    ...
    protected function _getReviewsMainQuery() {
        ...
        $query = "SELECT * FROM _DBPRF_product_comment WHERE id_product_comment > {$id_src} ORDER BY id_product_comment ASC LIMIT {$limit}";
        return $query;
    }
    ...
    public function convertReview($review, $reviewsExt) {
        ...
        $product_mage_id = $this->getMageIdProduct($review['id_product']);
        if (!$product_mage_id) {
            return array(
                'result' => 'warning',
                'msg' => $this->consoleWarning("Review Id = {$review['id_product_comment']} import failed. Error: Product Id = {$review['id_product']} not imported!")
            );
        }
        ...
        $data['status_id'] = ($review['validate'] == 0) ? 3 : 1;
        $data['title'] = $review['title'];
        $data['detail'] = $review['content'];
        $data['entity_id'] = 1;
        $data['stores'] = array($store_id);
        $data['customer_id'] = ($this->getMageIdCustomer($review['id_customer'])) ? $this->getMageIdCustomer($review['id_customer']) : null;
        $data['nickname'] = $review['customer_name'];
        $data['rating'] = $review['grade'];
        $data['created_at'] = $review['date_add'];
        $data['review_id_import'] = $review['id_product_comment'];
...
############################################################ Extend function ##################################
    ...
    protected function _convertProduct($product, $productsExt, $parent_id = null) {
        ...
        $proTier = $this->getListFromListByField($productsExt['object']['specific_price'], 'id_product', $product['id_product']);
        if ($proTier && !$parent_id) {
            $price = $product['price'];
            foreach ($proTier as $row) {
                if ($row['from_quantity'] >= 2) {
                    if ($row['reduction_type'] == 'amount') {
                        $tier_price = $price - $row['reduction'];
                    } else {
                        $tier_price = $price - ($row['reduction'] * $price);
                    }
                    if ($row['price'] > 0) {
                        $tier_price = $row['price'];
                    }
                    $value = array(
                        'website_id' => 0,
                        'cust_group' => Mage_Customer_Model_Group::CUST_GROUP_ALL,
                        'price_qty' => $row['from_quantity'],
                        'price' => $tier_price,
                    );
                    $tier_prices[] = $value;
                } else {
                    if ($row['reduction_type'] == 'amount') {
                        $special_price = $price - $row['reduction'];
                    } else {
                        $special_price = $price - ($row['reduction'] * $price);
                    }
                    if ($row['price'] > 0) {
                        $special_price = $row['price'];
                    }
                    $pro_data['special_price'] = $special_price;
                    $pro_data['special_from_date'] = ($row['from'] == "0000-00-00 00:00:00") ? '' : $row['from'];
                    $pro_data['special_to_date'] = ($row['to'] == "0000-00-00 00:00:00") ? '' : $row['to'];
                }
            }
        ...
        if ($product['id_tax_rules_group'] != 0 && $tax_pro_id = $this->getMageIdTaxProduct($product['id_tax_rules_group'])) {
        ...
        $pro_data['visibility'] = isset($visibility[$product['visibility']]) ? $visibility[$product['visibility']] : 4;
        if (!$parent_id) {
            $product_quantity_all = $this->getListFromListByField($productsExt['object']['stock_available'], 'id_product', $product['id_product']);
            $product_quantity = $this->getRowValueFromListByField($product_quantity_all, 'id_product_attribute', '0', 'quantity');
        } else {
            $product_quantity = $this->getRowValueFromListByField($productsExt['object']['stock_available'], 'id_product_attribute', $product['id_product_attribute'], 'quantity');
        }        
        ...
        if (!$parent_id) {
            ...
            if ($images) {
                ...
                $imagesDesc = $this->getListFromListByField($productsExt['object']['image_lang'], 'id_image', $image_cover);
                $images_label = $this->getRowValueFromListByField($imagesDesc, 'id_lang', $this->_notice['config']['default_lang'], 'legend');
                $image_path_ps = $this->_getImagePath($image_cover, $product['id_product']);
                if (!$parent_id && $image_path = $this->downloadImage($this->getUrlSuffix($this->_notice['config']['image_product']), $image_path_ps['legacy'], 'catalog/product', false, true)) {
                    $pro_data['image_import_path'] = array('path' => $image_path, 'label' => $images_label);
                } elseif (!$parent_id && $image_path = $this->downloadImage($this->getUrlSuffix($this->_notice['config']['image_product']), $image_path_ps['not_legacy'], 'catalog/product', false, true)) {
                    $pro_data['image_import_path'] = array('path' => $image_path, 'label' => $images_label);
                } elseif ($image_path = $this->downloadImage($this->getUrlSuffix($this->_notice['config']['image_product']), $image_path_ps['large'], 'catalog/product', false, true)) {
                    $pro_data['image_import_path'] = array('path' => $image_path, 'label' => $images_label);
                }
                ...
                foreach ($images_gallery_ps as $gallery) {
                    if ($gallery_path = $this->downloadImage($this->getUrlSuffix($this->_notice['config']['image_product']), $gallery['legacy'], 'catalog/product', false, true)) {
                        $pro_data['image_gallery'][] = array('path' => $gallery_path, 'label' => $images_label);
                    } elseif ($gallery_path = $this->downloadImage($this->getUrlSuffix($this->_notice['config']['image_product']), $gallery['not_legacy'], 'catalog/product', false, true)) {
                        $pro_data['image_gallery'][] = array('path' => $gallery_path, 'label' => $images_label);
                    }
                }
            }
        ...
        /*if ($parent_id) {
            $childImgs = $this->getListFromListByField($productsExt['object']['product_attribute_image'], 'id_product_attribute', $product['id_product_attribute']);
            foreach ($childImgs as $key => $img) {
                $image_path_ps = $this->_getImagePath($img['id_image'], $product['id_product']);
                $image_path = $this->downloadImage($this->getUrlSuffix($this->_notice['config']['image_product']), $image_path_ps['legacy'], 'catalog/product', false, true);
                if (!$image_path) {
                    $image_path = $this->downloadImage($this->getUrlSuffix($this->_notice['config']['image_product']), $image_path_ps['not_legacy'], 'catalog/product', false, true);
                }
                if ($image_path) {
                    if ($key == 0) {
    ...
    /**
     * Get images path of specific category in Prestashop
     */
    protected function _getCategoryImagePath($category_id) {
        $category_images = array();
        $category_images['image'] = 'c/' . $category_id . '.jpg';
        $category_images['thumbnail'] = 'c/' . $category_id . '-medium_default.jpg';
        return $category_images;
    }

    /**
     * Get children products and name
     */
    protected function _getChildrenProduct($product, $productsExt) {
        $product_id = $product['id_product'];
        $product_tax = $product['id_tax_rules_group'];
        $price = $product['price'];
        ...
        foreach ($child_products as $key => $child_product) {
            ...
            if (!$child_product['reference']) {
                $child_products[$key]['reference'] = $product['reference'] . '-' . $key;
            }
            $child_products[$key]['sku'] = $sku;
            $child_products[$key]['id_tax_rules_group'] = $product_tax;
    ...
    protected function _importChildrenProduct($parent_id, $children, $productsExt) {
        ...
        foreach ($children as $row) {
            ...
            foreach ($pro_attr as $value) {
                ...
                $attr_label = $attr_import['config']['frontend_label'][0];
                $attr_after = $this->_process->attribute($attr_import['config'], $attr_import['edit']);
                if(!$attr_after) {var_dump($attr_import);return false;}
    ...
    protected function _getImagePath($id_image, $id_product) {
        $path = $this->_getImgFolderStatic($id_image);
        $image_path['legacy'] = 'p/' . $path . $id_image . '.jpg';
        $image_path['not_legacy'] = 'p/' . $id_product . '-' . $id_image . '.jpg';
        $image_path['large'] = 'p/' . $path . $id_image . '-large_default.jpg';
        return $image_path;
    }
    ...
    protected function _getImgFolderStatic($id_image) {
    ...
    public function getAllTaxes()
    {
        ...
        $query = "SELECT * FROM _DBPRF_tax_rules_group ORDER BY id_tax_rules_group ASC";
        $data = $this->_getDataImport($this->_getUrlConnector('query'), array(
            'query' => $query,
        ));
        if(!$data || $data['result'] != 'success'){
    ...
    public function getAllCategories()
    {
        ...
        $query = "SELECT * FROM _DBPRF_category WHERE id_category != {$this->_notice['extend']['highest_root']} AND id_category != {$this->_notice['extend']['root_category']} ORDER BY id_category ASC";
        $data = $this->_getDataImport($this->_getUrlConnector('query'), array(
            'query' => $query,
        ));
        if(!$data || $data['result'] != 'success'){
    ...
    public function getAllReviews()
    {
        ...
        $query = "SELECT * FROM _DBPRF_product_comment ORDER BY id_product_comment ASC";
        $data = $this->_getDataImport($this->_getUrlConnector('query'), array(
            'query' => $query,
        ));
        if(!$data || $data['result'] != 'success'){
