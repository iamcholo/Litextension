 ...
 */
class LitExtension_CartMigration_Model_Cart_Loadedcommercev6b2b extends LitExtension_CartMigration_Model_Cart {
    ...
    public function displayConfig() {
        ...
        $data = $this->_getDataImport($this->_getUrlConnector('query'), array(
            ...
            "query" => serialize(array(
                ...
                "customers_groups" => "SELECT * FROM _DBPRF_customers_groups"
            ))
        ));
        if (!$data || $data['result'] != 'success') {
            return $this->errorConnector();
    ...
    protected function _getProductsExtQuery($products) {
        ...
        $ext_query = array(
            ...
            'products_groups' => "SELECT * FROM _DBPRF_products_groups WHERE products_id IN {$pro_ids_query}",
    ...
    protected function _convertProduct($product, $productsExt, $parent_id = null) {
        ...
        if ($product['products_model']) {
            $pro_data['sku'] = $this->createProductSku(strtolower(str_replace(" ", "-", $product['products_model'])), $this->_notice['config']['languages']);
        } else {
            $pro_data['sku'] = $this->createProductSku(strtolower(str_replace(" ", "-", $pro_desc_def['products_name'])), $this->_notice['config']['languages']);
        }
        ...
        //Tier price
        $proTier = $this->getListFromListByField($productsExt['object']['products_groups'], 'products_id', $product['products_id']);
        if ($proTier && !$parent_id) {
            foreach ($proTier as $row) {
                if (!isset($this->_notice['config']['customer_group'][$row['customers_group_id']])) {
                    continue;
                }
                for ($i = 1; $i < 6; $i++) {
                    if ($row['customers_group_price' . $i] > 0) {
                        $value = array(
                            'website_id' => 0,
                            'cust_group' => $this->_notice['config']['customer_group'][$row['customers_group_id']],
                            'price_qty' => $product['products_price' . $i . '_qty'],
                            'price' => $row['customers_group_price' . $i]
                        );
                        $tier_prices[] = $value;
                    }
                }
        ...
        if ($proTier && !$parent_id) {
            foreach ($proTier as $row) {
                ...
                if ((int)$row['customers_group_price']) {
                    $value = array(
                        'website_id' => 0,
                        'cust_group' => $this->_notice['config']['customer_group'][$row['customers_group_id']],
                        'price' => $row['customers_group_price']
                    );
                    $group_prices[] = $value;
                }
            }
        ...
        for ($i = 1; $i < 6; $i++) {
            if ($product['products_price' . $i] > 0) {
                $value = array(
                    'website_id' => 0,
                    'cust_group' => Mage_Customer_Model_Group::CUST_GROUP_ALL,
                    'price_qty' => $product['products_price' . $i . '_qty'],
                    'price' => $product['products_price' . $i]
                );
                $tier_prices[] = $value;
            }
        }
        $pro_data['tier_price'] = $tier_prices;
        $pro_data['group_price'] = $group_prices;
        //End
...
############################################################ Extend function ##################################
    ...
    public function getAllProducts()
    {
        ...
        $query = "SELECT * FROM _DBPRF_productsWHERE products_parent_id = 0 ORDER BY products_id ASC";
        $data = $this->_getDataImport($this->_getUrlConnector('query'), array(
            'query' => $query,
        ));
        if(!$data || $data['result'] != 'success'){
