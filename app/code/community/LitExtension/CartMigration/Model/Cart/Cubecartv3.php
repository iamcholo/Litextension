...
class   LitExtension_CartMigration_Model_Cart_Cubecartv3 extends LitExtension_CartMigration_Model_Cart{
    ...
    */
    ...
    protected function _getTaxesExtQuery($taxes){
       $ext_query = array(
            'tax_countries' => "SELECT * FROM _DBPRF_config WHERE name = 'config'",
            'iso_counties' => "SELECT * FROM _DBPRF_iso_counties",
    ...
    public function convertTax($tax, $taxesExt){
        ...
        $taxCountry = $taxesExt['object']['tax_countries'][0]['array'];
        $a = unserialize($taxCountry);
        $d = array();
        ...
        $country_iso = $this->getRowValueFromListByField($taxesExt['object']['iso_countries'], 'id', $d['taxCountry'], 'iso');
        $tax_rate_data['tax_country_id'] = $country_iso;
        if(!$d['taxCounty']){
            $tax_rate_data['tax_region_id'] = 0;
        } else {
            $zone_name = $this->getRowValueFromListByField($taxesExt['object']['iso_counties'], 'id', $d['taxCounty'], 'name');
            $tax_rate_data['tax_region_id'] = $this->getRegionId($zone_name, $country_iso);
        }
        $tax_rate_data['zip_is_range'] = 0;
        $tax_rate_data['tax_postcode'] = "*";
        $tax_rate_data['rate'] = $tax['percent'];
    ...
    public function convertProduct($product, $productsExt){
        ...
        $pro_desc_def = $this->getRowFromListByField($proDesc, 'prod_lang',$this->_notice['config']['default_lang']);
        $pro_data = array();
        ...
        if ($product['image']) {
          $gallery_path = $this->downloadImage($this->getUrlSuffix($this->_notice['config']['image_product']), $product['image'], 'catalog/product', false, true);
    ...
    public function convertOrder($order, $ordersExt){
        ...
        $order_subtotal = 0;
            ...
            foreach($orderPro as $order_pro){
                ...
                $order_subtotal = $cart['row_total']*$cart['qty_ordered'];
        ...
        $order_data['discount_amount'] = 0;// $order['discount'];
        $order_data['base_discount_amount'] = 0;// $order['discount'];
        $order_data['grand_total'] = $this->incrementPriceToImport($order['prod_total']);
        $order_data['base_grand_total'] = $order['subtotal'];
        $order_data['base_total_invoiced'] = $order['subtotal'];
        $order_data['total_paid'] = $order['subtotal'];
        $order_data['base_total_paid'] = $order['subtotal'];
