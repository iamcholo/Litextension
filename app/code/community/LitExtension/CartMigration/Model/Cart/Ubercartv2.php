 ...
 */
class LitExtension_CartMigration_Model_Cart_Ubercartv2
    ...
    public function checkRecent()
    {
        $data = $this->_getDataImport($this->_getUrlConnector('query'), array(
            ...
            'query' => serialize(array(
                'taxes' => "SELECT COUNT(1) FROM _DBPRF_uc_taxes WHERE WHERE tax_id > {$this->_notice['taxes']['id_src']}",
                'categories' => "SELECT COUNT(1) FROM _DBPRF_term_data WHERE tid > {$this->_notice['categories']['id_src']}",
                'products' => "SELECT COUNT(1) FROM _DBPRF_node WHERE type = 'product' AND nid > {$this->_notice['products']['id_src']}",
                ...
                'reviews' => "SELECT COUNT(1) FROM _DBPRF_comments WHERE  AND cid > {$this->_notice['reviews']['id_src']}"
    ...
    public function displayImport(){
        ...
        $data = $this->_getDataImport($this->_getUrlConnector('query'), array(
            ...
            'query' => serialize(array(
                ...
                'orders' => "SELECT COUNT(1) FROM _DBPRF_uc_orders WHERE order_id > {$this->_notice['orders']['id_src']}",
                'reviews' => "SELECT COUNT(1) FROM _DBPRF_comments WHERE cid > {$this->_notice['reviews']['id_src']}"
            ))
        ));
        if(!$data || $data['result'] != 'success'){
            return $this->errorConnector();
    ...
    protected function _getCategoriesMainQuery(){
        ...
        $query = "SELECT * FROM _DBPRF_term_data as td
                          LEFT JOIN _DBPRF_term_hierarchy AS th ON td.tid = th.tid
    ...
    public function getProductsExt($products){
        ...
        $ext_query = array(
            "products" => "SELECT nd.*,up.*,nr.body FROM _DBPRF_uc_products as up
                            LEFT JOIN _DBPRF_node as nd ON up.nid = nd.nid
                            LEFT JOIN _DBPRF_node_revisions as nr ON nr.nid = up.nid",
            ...
            "product_categories" => "SELECT * FROM _DBPRF_term_node as pc",
    ...
    public function convertProduct($product, $productsExt){
        ...
        $proCat = $this->getListFromListByField($productsExt['object']['product_categories'], 'nid', $product['nid']);

        if($proCat){
            foreach($proCat as $pro_cat){
                $cat_id = $this->getMageIdCategory($pro_cat['tid']);
                if($cat_id){
                    $categories[] = $cat_id;
                }
            }
        ...
        if($data['type_id'] == 'simple'){
            ...
            if($customoptions){
                ...
                foreach($customoptions as $option){
                    ...
                    $attr_display = $this->getRowValueFromListByField($productsExt['object']['attributes'],'aid',$option['aid'], 'display');
...
//
        ...
        $data['uc_payment_method'] = $order['payment_method'];
    ...
    protected function _getReviewsMainQuery(){
        ...
        $query = "SELECT * FROM _DBPRF_comments AS cm
                    WHERE cm.cid > {$id_src} ORDER BY cm.cid ASC LIMIT {$limit}";
...
###################################### Extend Function #################################################################
    ...
    protected function _convertProduct($product, $productsExt, $child_data){
        ...
        $description = $this->getRowValueFromListByField($productsExt['object']['products'], 'nid', $product['nid'], 'body');
        ...
        if(count($images)){
            ...
            foreach($images as $image){
                $img_src = $this->getRowValueFromListByField($productsExt['object']['product_images'], 'fid', $image['fid'], 'filename');
                if($key == 0){
