...
class LitExtension_CartMigration_Model_Cart_Wpecommercev37
    extends LitExtension_CartMigration_Model_Cart
{
    ...
    public function checkRecent()
    {
        $data = $this->_getDataImport($this->_getUrlConnector('query'), array(
            ...
            'query' => serialize(array(
                'taxes' => "SELECT COUNT(1) FROM _DBPRF_options AS opt,_DBPRF_wpsc_currency_list AS cur WHERE opt.option_name = 'base_country' AND cur.isocode = opt.option_value",
                'manufacturers' => "SELECT COUNT(1) FROM _DBPRF_wpsc_product_categories WHERE group_id = 2 AND id > {$this->_notice['manufacturers']['id_src']}",
                'categories' => "SELECT COUNT(1) FROM _DBPRF_wpsc_product_categories WHERE group_id = 1 AND id > {$this->_notice['categories']['id_src']}",
                'products' => "SELECT COUNT(1) FROM _DBPRF_wpsc_product_list WHERE publish = 1 AND active = 1 AND id > {$this->_notice['products']['id_src']}",
    ...
    public function displayConfig()
    {
        ...
        $data = $this->_getDataImport($this->_getUrlConnector('query'), array(
            ...
            "query" => serialize(array(
                ...
                "orders_status" => "SELECT * FROM _DBPRF_wpsc_purchase_statuses WHERE active = 1",
    ...
    protected function _getTaxesMainQuery(){
        $query = "SELECT *, 0 AS check_region_table FROM _DBPRF_options AS opt,_DBPRF_wpsc_currency_list AS cur
    ...
    protected function _getTaxesExtQuery($taxes){
        ...
        $ext_query = array(
            'regions_tax' => "SELECT *,1 AS check_region_table FROM _DBPRF_wpsc_region_tax WHERE country_id IN {$country_ids_query} AND tax != 0"
        );
        return $ext_query;
    }
    ...
    protected function _getManufacturersMainQuery(){
        ...
        $query = "SELECT * FROM _DBPRF_wpsc_product_categories WHERE group_id = 2 AND id > {$id_src} ORDER BY id ASC LIMIT {$limit}";
        return $query;
    }
    ...
    protected function _getCategoriesMainQuery(){
        ...
        $query = "SELECT * FROM _DBPRF_wpsc_product_categories WHERE group_id = 1 AND id > {$id_src} ORDER BY id ASC LIMIT {$limit}";
        return $query;
    }
    ...
    protected function _getProductsMainQuery(){
        ...
        $query = "SELECT * FROM _DBPRF_wpsc_product_list WHERE publish = 1 AND active = 1 AND id > {$id_src} ORDER BY id ASC LIMIT {$limit}";
        return $query;
    }
    ...
    protected function _getProductsExtQuery($products){
        ...
        $images_ids = $this->duplicateFieldValueFromList($products['object'], 'image');
        ...
        $ext_query = array(
            ...
            'categories_products' => "SELECT * FROM _DBPRF_wpsc_item_category_assoc AS ica
                                        LEFT JOIN _DBPRF_wpsc_product_categories AS pc ON pc.id = ica.category_id
                                        WHERE ica.product_id IN {$pro_ids_query}",
            'images_products' => "SELECT * FROM _DBPRF_wpsc_product_images WHERE id IN {$images_ids_con}",
            'variation_combinations' => "SELECT * FROM _DBPRF_wpsc_variation_combinations AS vc
                                                 LEFT JOIN _DBPRF_wpsc_variation_values AS vv ON vv.id = vc.value_id
                                                 LEFT JOIN _DBPRF_wpsc_variation_properties AS vp ON vp.id = vc.priceandstock_id
                                                 WHERE vc.product_id IN {$pro_ids_query}"
        );
        return $ext_query;
    }
    ...
    protected function _getProductsExtRelQuery($products, $productsExt){
        $variation_ids = $this->duplicateFieldValueFromList($productsExt['object']['variation_combinations'], 'variation_id');
        $variationIdsCon = $this->arrayToInCondition($variation_ids);
        $ext_rel = array(
            'product_variations' => "SELECT * FROM _DBPRF_wpsc_product_variations WHERE id IN {$variationIdsCon}"
        );
        return $ext_rel;
    }
    ...
    public function convertProduct($product, $productsExt){
        ...
        if($product['image'] != '' && $image_path = $this->downloadImage($this->getUrlSuffix($this->_notice['config']['image_product']), $this->getRowValueFromListByField($productsExt['object']['images_products'], 'id', $product['image'], 'image'), 'catalog/product', false, true)){
    ...
    protected function _getCustomersExtQuery($customers){
        ...
        $ext_query = array(
            ...
            'checkout_forms' => "SELECT * FROM _DBPRF_wpsc_checkout_forms WHERE `active` = '1' AND unique_name IN ('billingfirstname','billinglastname','billingaddress','billingcity','billingcountry','billingpostcode','billingemail','billingphone','shippingfirstname','shippinglastname','shippingcountry','shippingstate','shippingaddress','shippingpostcode','shippingcity') ORDER BY `order`"
        );
        return $ext_query;
    }
    ...
    protected function _getCustomersExtRelQuery($customers, $customersExt){
        $billCountryFormIds = $this->getRowValueFromListByField($customersExt['object']['checkout_forms'], 'unique_name', 'billingcountry', 'id');
        $shipCountryFormIds = $this->getRowValueFromListByField($customersExt['object']['checkout_forms'], 'unique_name', 'shippingcountry', 'id');
        $usr_profile_list = $this->getListFromListByField($customersExt['object']['user_meta'], 'meta_key', 'wpshpcrt_usr_profile');
        $region_ids = array();
        if($usr_profile_list){
            foreach($usr_profile_list as $usr_profile){
                $tmp = unserialize($usr_profile['meta_value']);
                if($billCountryFormIds){
                    if(isset($tmp[$billCountryFormIds]) && is_array($tmp[$billCountryFormIds])){
                        if(isset($tmp[$billCountryFormIds][1])) $region_ids[] = $tmp[$billCountryFormIds][1];
                    }
                }
                if($shipCountryFormIds){
                    if(isset($tmp[$shipCountryFormIds]) && is_array($tmp[$shipCountryFormIds])){
                        if(isset($tmp[$shipCountryFormIds][1])) $region_ids[] = $tmp[$shipCountryFormIds][1];
                    }
                }
            }
        }
        $region_ids = array_unique($region_ids);
        $region_ids_con = $this->arrayToInCondition($region_ids);
        $ext_rel = array(
            'region_tax' => "SELECT * FROM _DBPRF_wpsc_region_tax WHERE id IN {$region_ids_con}"
        );
        return $ext_rel;
    }
    ...
    public function afterSaveCustomer($customer_mage_id, $data, $customer, $customersExt){
        ...
        $user_meta = $this->getListFromListByField($customersExt['object']['user_meta'], 'user_id', $customer['ID']);
        $billFirstNameId = $this->getRowValueFromListByField($customersExt['object']['checkout_forms'], 'unique_name', 'billingfirstname', 'id');
        $billLastNameId = $this->getRowValueFromListByField($customersExt['object']['checkout_forms'], 'unique_name', 'billinglastname', 'id');
        $billCountryId = $this->getRowValueFromListByField($customersExt['object']['checkout_forms'], 'unique_name', 'billingcountry', 'id');
        $billAddressId = $this->getRowValueFromListByField($customersExt['object']['checkout_forms'], 'unique_name', 'billingaddress', 'id');
        $billPostCodeId = $this->getRowValueFromListByField($customersExt['object']['checkout_forms'], 'unique_name', 'billingpostcode', 'id');
        $billCityId = $this->getRowValueFromListByField($customersExt['object']['checkout_forms'], 'unique_name', 'billingcity', 'id');
        $billPhoneId = $this->getRowValueFromListByField($customersExt['object']['checkout_forms'], 'unique_name', 'billingphone', 'id');

        $shipFirstNameId = $this->getRowValueFromListByField($customersExt['object']['checkout_forms'], 'unique_name', 'shippingfirstname', 'id');
        $shipLastNameId = $this->getRowValueFromListByField($customersExt['object']['checkout_forms'], 'unique_name', 'shippinglastname', 'id');
        $shipCountryId = $this->getRowValueFromListByField($customersExt['object']['checkout_forms'], 'unique_name', 'shippingcountry', 'id');
        $shipStateId = $this->getRowValueFromListByField($customersExt['object']['checkout_forms'], 'unique_name', 'shippingstate', 'id');
        $shipAddressId = $this->getRowValueFromListByField($customersExt['object']['checkout_forms'], 'unique_name', 'shippingaddress', 'id');
        $shipPostCodeId = $this->getRowValueFromListByField($customersExt['object']['checkout_forms'], 'unique_name', 'shippingpostcode', 'id');
        $shipCityId = $this->getRowValueFromListByField($customersExt['object']['checkout_forms'], 'unique_name', 'shippingcity', 'id');
        ...
        if($user_meta){
            ...
            if(is_array($address_meta)){
                ...
                $address_billing['firstname'] = isset($address_meta[$billFirstNameId]) ? $address_meta[$billFirstNameId] : '';
                $address_billing['lastname'] = isset($address_meta[$billLastNameId]) ? $address_meta[$billLastNameId] : '';
                if(isset($address_meta[$billCountryId]) && is_array($address_meta[$billCountryId])){
                    $address_billing['country_id'] = isset($address_meta[$billCountryId][0]) ? $address_meta[$billCountryId][0] : '';
                    if(isset($address_meta[$billCountryId][1])) $bill_region = $address_meta[$billCountryId][1];
                }elseif(isset($address_meta[$billCountryId])){
                    $address_billing['country_id'] = $address_meta[$billCountryId];
                }
                if(isset($bill_region) && is_numeric($bill_region) && isset($address_billing['country_id'])){
                    $bill_region_code = $this->getRowValueFromListByField($customersExt['object']['region_tax'], 'id', $bill_region, 'code');
                    if($bill_region_code) $address_billing['region_id'] = $this->_getRegionIdByCode($bill_region_code, $address_billing['country_id']);
                }elseif(isset($bill_region)){
                    $address_billing['region'] = $bill_region;
                }
                $address_billing['street'][0] = isset($address_meta[$billAddressId]) ? $address_meta[$billAddressId] : '';
                $address_billing['postcode'] = isset($address_meta[$billPostCodeId]) ? $address_meta[$billPostCodeId] : '';
                $address_billing['city'] = isset($address_meta[$billCityId]) ? $address_meta[$billCityId] : '';
                $address_billing['telephone'] = isset($address_meta[$billPhoneId]) ? $address_meta[$billPhoneId] : '';
                ...
                $address_shipping['firstname'] = isset($address_meta[$shipFirstNameId]) ? $address_meta[$shipFirstNameId] : '';
                $address_shipping['lastname'] = isset($address_meta[$shipLastNameId]) ? $address_meta[$shipLastNameId] : '';
                if(isset($address_meta[$shipCountryId]) && is_array($address_meta[$shipCountryId])){
                    $address_shipping['country_id'] = isset($address_meta[$shipCountryId][0]) ? $address_meta[$shipCountryId][0] : '';
                    if(isset($address_meta[$shipCountryId][1])) $ship_region = $address_meta[$shipCountryId][1];
                }elseif(isset($address_meta[$shipCountryId])){
                    $address_shipping['country_id'] = $address_meta[$shipCountryId];
                }
                if(isset($ship_region) && is_numeric($ship_region) && isset($address_shipping['country_id'])){
                    $ship_region_code = $this->getRowValueFromListByField($customersExt['object']['region_tax'], 'id', $ship_region, 'code');
                    if($ship_region_code) $address_shipping['region_id'] = $this->_getRegionIdByCode($ship_region_code, $address_shipping['country_id']);
                }elseif(isset($address_meta[$shipStateId])){
                    $address_shipping['region'] = $address_meta[$shipStateId] ? $address_meta[$shipStateId] : '';
                }
                $address_shipping['street'][0] = isset($address_meta[$shipAddressId]) ? $address_meta[$shipAddressId] : '';
                $address_shipping['postcode'] = isset($address_meta[$shipPostCodeId]) ? $address_meta[$shipPostCodeId] : '';
                $address_shipping['city'] = isset($address_meta[$shipCityId]) ? $address_meta[$shipCityId] : '';
    ...
    public function getOrdersExt($orders){
        ...
        $ext_query = array(
            'submit_form_data' => "SELECT * FROM _DBPRF_wpsc_submited_form_data AS sfd
                                LEFT JOIN _DBPRF_wpsc_checkout_forms AS cdf ON cdf.id = sfd.form_id
                                WHERE sfd.log_id IN {$order_ids_con}",
            'cart_contents' => "SELECT * FROM _DBPRF_wpsc_cart_contents WHERE purchaseid IN {$order_ids_con}",
        ...
        if($ext_query) {
            ...
            $countryList = $this->_getListFromListByFieldInArray($ordersExt['object']['submit_form_data'], 'unique_name', array('billingcountry', 'shippingcountry'));
            $region_ids = array();
            if ($countryList) {
                foreach ($countryList as $countryData) {
                    $tmp = unserialize($countryData['value']);
                    if ($tmp && is_array($tmp)) {
            ...
            $region_ids_con = $this->arrayToInCondition($region_ids);
            $ext_rel_query = array(
                'products_meta' => "SELECT pl.*, pm.meta_value FROM _DBPRF_wpsc_product_list AS pl
                                LEFT JOIN _DBPRF_wpsc_productmeta AS pm ON pm.product_id = pl.id
                                AND pm.meta_key = 'sku'
                                WHERE pl.id IN {$pro_ids_con}",
                'cart_item_variations' => "SELECT * FROM _DBPRF_wpsc_cart_item_variations AS civ
                                    LEFT JOIN _DBPRF_wpsc_variation_values AS vv ON vv.id = civ.value_id
    ...
    public function convertOrder($order, $ordersExt){
        ...
        $bill_country = unserialize($this->getRowValueFromListByField($submit_form, 'unique_name', 'billingcountry', 'value'));
        if($bill_country && is_array($bill_country)){
            $address_billing['country_id'] = isset($bill_country[0]) ? $bill_country[0] : '';
            if(isset($bill_country[1]) && $bill_country[1] && $address_billing['country_id']){
                $bill_region_code = $this->getRowValueFromListByField($ordersExt['object']['region_tax'], 'id', $bill_country[1], 'code');
                if($bill_region_id = $this->_getRegionIdByCode($bill_region_code, $address_billing['country_id'])){
                    ...
                    $address_billing['region'] = $bill_country[1];
                }
            }
        }
        ...
        $ship_country = unserialize($this->getRowValueFromListByField($submit_form, 'unique_name', 'shippingcountry', 'value'));
        if($ship_country && is_array($ship_country)){
            $address_shipping['country_id'] = isset($ship_country[0]) ? $ship_country[0] : '';
            if(isset($ship_country[1]) && $ship_country[1] && $address_shipping['country_id']){
                $ship_region_code = $this->getRowValueFromListByField($ordersExt['object']['region_tax'], 'id', $ship_country[1], 'code');
                if($ship_region_id = $this->_getRegionIdByCode($ship_region_code, $address_shipping['country_id'])){
                    $address_shipping['region_id'] = $ship_region_id;
                }else{
                    $address_shipping['region'] = $ship_region_id[1];
                }
            }
        }
        $address_shipping['telephone'] = $address_billing['telephone'] ? $address_billing['telephone'] : '';
        ...
        if($orderPro){
            foreach($orderPro as $item){
                ...
                $cart['price'] = number_format($item['price'], 2, '.', ',');
                $cart['original_price'] =  $cart['price'];
                $cart['tax_amount'] = number_format($item['tax_charged'], 2, '.', ',');
                $cart['qty_ordered'] = $item['quantity'];
                $cart['tax_percent'] = $item['gst'];
                $cart['row_total'] = $item['price']*$item['quantity'] + $item['pnp'] + $item['tax_charged']*$item['quantity'];
...
############################## Extend Function ################################################################
    ...
    protected function _createGrandPriceProduct($table_product_prices){
        $tierPrices = array();
        if(is_array($table_product_prices) && isset($table_product_prices['quantity']) && isset($table_product_prices['table_price'])){
    ...
    protected function _importCategoryParent($parent_id){
        $categories = $this->_getDataImport($this->_getUrlConnector('query'), array(
            'query' => "SELECT * FROM _DBPRF_wpsc_product_categories
    ...
    public function getAllManufacturers()
    {
        ...
        $query = "SELECT * FROM _DBPRF_wpsc_product_categories WHERE group_id = 2 ORDER BY id ASC";
        $data = $this->_getDataImport($this->_getUrlConnector('query'), array(
            'query' => $query,
        ));
        if(!$data || $data['result'] != 'success'){
    ...
    public function getAllCategories()
    {
        ...
        $query = "SELECT * FROM _DBPRF_wpsc_product_categories WHERE group_id = 1 ORDER BY id ASC";
        $data = $this->_getDataImport($this->_getUrlConnector('query'), array(
            'query' => $query,
        ));
        if(!$data || $data['result'] != 'success'){
    ...
    public function getAllProducts()
    {
        ...
        $query = "SELECT * FROM _DBPRF_wpsc_product_list WHERE publish = 1 AND active = 1 ORDER BY id ASC";
        $data = $this->_getDataImport($this->_getUrlConnector('query'), array(
            'query' => $query,
        ));
        if(!$data || $data['result'] != 'success'){
