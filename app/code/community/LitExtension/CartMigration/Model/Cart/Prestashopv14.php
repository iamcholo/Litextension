 ...
 */
class LitExtension_CartMigration_Model_Cart_Prestashopv14 extends LitExtension_CartMigration_Model_Cart {
    ...
    public function getProductsExt($products) {
        ...
        $ext_rel_query = array(
            ...
            'product_attribute_combination' => "SELECT pac.*, a.id_attribute_group
    ...
    public function convertOrder($order, $ordersExt) {
        ...
        foreach ($orderPro as $order_pro) {
            ...
            $cart['original_price'] = isset($order_pro['original_product_price']) ? $order_pro['original_product_price'] : $order_pro['product_price'];
        ...
        $order_data['tax_amount'] = $order['total_products_wt'] - $order['total_products'];
        $order_data['base_tax_amount'] = $order['total_products_wt'] - $order['total_products'];
...
############################################################ Extend function ##################################
        ...
        if ($proTier && !$parent_id) {
            ...
            foreach ($proTier as $row) {
                if ($row['from_quantity'] >= 2) {
                    ...
                    $pro_data['special_from_date'] = $this->_cookSpecialDate($row['from']);
                    $pro_data['special_to_date'] = $this->_cookSpecialDate($row['to']);
                }
            }
    ...
    protected function _importChildrenProduct($parent_id, $children, $productsExt) {
        ...
        foreach ($children as $row) {
            ...
            //$product = array_merge($product, $child_attr[$row['id_product_attribute']]);
            ...
            if ($pro_import['result'] !== 'success')
                return false;
            //$this->productSuccess($row['id_product_attribute'], $pro_import['mage_id']);
    ...
    protected function _getImagePath($id_image, $id_product) {
        ...
        $image_path['large'] = 'p/' . $path . $id_image . '-large.jpg';
        return $image_path;
    }
