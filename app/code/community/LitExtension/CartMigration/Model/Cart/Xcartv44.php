...
class LitExtension_CartMigration_Model_Cart_Xcartv44
        ...
        $data = $this->_getDataImport($this->_getUrlConnector('query'), array(
            ...
            "query" => serialize(array(
                ...
                "orders_status" => "SELECT * FROM _DBPRF_languages AS lg
                                    WHERE lg.code = '{$this->_notice['config']['default_lang']}'
                                    AND lg.name IN ('lbl_not_finished','lbl_queued','lbl_pre_authorized','lbl_processed','lbl_backordered','lbl_declined','lbl_failed','lbl_complete')",
    ...
    public function getProductsExt($products){
        ...
        $ext_query = array(
            ...
            'images_z' => "SELECT * FROM _DBPRF_images_Z AS img_d WHERE img_d.id IN {$proIds_in_query} ",
    ...
    public function convertProduct($product, $productsExt){
        ...
        $images_list_z = $this->getListFromListByField($productsExt['object']['images_z'], 'id', $product['productid']);
        ...
        if($images_list_z){
            foreach($images_list_z as $gallery){
                ...
                if($gallery_path = $this->downloadImage($this->getUrlSuffix($this->_notice['config']['image_product']), $img_z_path, 'catalog/product', false, true)){
                    $pro_data['image_gallery'][] = array('path' => $gallery_path, 'label' => $gallery['alt'] ? $gallery['alt'] : $pro_data['name']) ;
                }
            }
        }
    ...
    public function convertOrder($order, $ordersExt){
        ...
        $address_billing['fax'] = $order['b_fax'];
        ...
        $address_shipping['fax'] = $order['s_fax'];
