...
class LitExtension_Wysiwyg_Adminhtml_UtilsController extends Mage_Adminhtml_Controller_Action
{
...
//            require_once('php_image_magician.php');
    ...
    function fix_path($path, $transliteration, $convert_spaces = false, $replace_with = "_")
    {
        ...
        $str = $this->fix_filename($info['filename'], $transliteration, $convert_spaces, $replace_with);
        if ($tmp_path != "")
    ...
    function config_loading($current_path, $fld)
    {
        ...
        echo "!!!!" . $parent = $this->fix_dirname($fld);
        if ($parent != "." && !empty($parent)) {
            $this->config_loading($current_path, $parent);
        }
...
// Need more feedback from users and refactor if needed
    function rrename($source, $destination, $is_rec = FALSE)
    {
        if (is_dir($source)) {
            ...
            foreach ($files as $file) {
                if ($file != "." && $file != "..") {
                    $this->rrename($source . DIRECTORY_SEPARATOR . $file, rtrim($destination, '/') . DIRECTORY_SEPARATOR . $file, TRUE);
                }
            }
        } else {
            if (file_exists($source)) {
            ...
            foreach ($files as $file) {
                if ($file != "." && $file != "..") {
                    if (is_dir($source . DIRECTORY_SEPARATOR . $file)) {
                        ...
                        $this->rchmod($source . DIRECTORY_SEPARATOR . $file, $mode, $rec_option, TRUE);
                    } else {
