...
class LitExtension_AjaxLogin_Model_Attributemanager extends Mage_Eav_Model_Entity_Attribute
{
    ...
    protected function _initAttributes(){
        ...
        $ajaxloginId  = $this->getId();
        $modelATTR = Mage::getModel('ajaxlogin/attributes');
        if ($ajaxloginId) {
            $modelATTR->load($ajaxloginId,'attribute_id');
        }
        Mage::register('current_attributes', $modelATTR);
        return $modelATTR;
    }

    protected function _afterSave(){
        $data = $this->getData();
        $acr = $this->_initAttributes();
        $acr->addData($data);
        $acr->save();
