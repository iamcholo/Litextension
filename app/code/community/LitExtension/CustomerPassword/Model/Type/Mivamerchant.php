...
class LitExtension_CustomerPassword_Model_Type_Mivamerchant
    extends LitExtension_CustomerPassword_Model_Type
{
    public function validatePassword($customer, $username, $password, $pw_hash){
        if(strpos($pw_hash, 'miva:') !== false){
            $pw_hash = str_replace('miva:', '',$pw_hash);
            $pw_hash = base64_decode($pw_hash);
            return self::verify($password, $pw_hash);
        }
        return false;
    }
