...
//        $this->getLayout()->getBlock('root')->setTemplate('le_sociallogin/fix.phtml');
    ...
    public function connectAction(){
        ...
        $pid = $attributeModel->getIdByCode('customer', 'le_sociallogin_pid');
        $ptoken = $attributeModel->getIdByCode('customer', 'le_sociallogin_ptoken');

        $aid = $attributeModel->getIdByCode('customer', 'le_sociallogin_aid');
        $atoken = $attributeModel->getIdByCode('customer', 'le_sociallogin_atoken');
        ...
        if($fid == false || $ftoken == false ||
            ...
            $pid == false || $ptoken == false ||
            $aid == false || $atoken == false
        ){
            ...
            if($pid == false){
                echo 'le_sociallogin_pid not exits<br />';
                $setup->addAttribute('customer', 'le_sociallogin_pid', array(
                    'type' => 'text',
                    'visible' => 0,
                    'required' => 0,
                    'user_defined' => 0,
                ));
                echo 'le_sociallogin_pid setup ok<br />';
            }
            if($ptoken == false){
                echo 'le_sociallogin_ptoken not exits<br />';
                $setup->addAttribute('customer', 'le_sociallogin_ptoken', array(
                    'type' => 'text',
                    'visible' => 0,
                    'required' => 0,
                    'user_defined' => 0,
                ));
                echo 'le_sociallogin_ptoken setup ok<br />';
            }

            if($aid == false){
                echo 'le_sociallogin_aid not exits<br />';
                $setup->addAttribute('customer', 'le_sociallogin_aid', array(
                    'type' => 'text',
                    'visible' => 0,
                    'required' => 0,
                    'user_defined' => 0,
                ));
                echo 'le_sociallogin_aid setup ok<br />';
            }
            if($atoken == false){
                echo 'le_sociallogin_atoken not exits<br />';
                $setup->addAttribute('customer', 'le_sociallogin_atoken', array(
                    'type' => 'text',
                    'visible' => 0,
                    'required' => 0,
                    'user_defined' => 0,
                ));
                echo 'le_sociallogin_atoken setup ok<br />';
            }
