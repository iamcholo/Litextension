...
class LitExtension_ACRegistration_IndexController extends Mage_Adminhtml_Controller_Action
{
    ...
    {
        $this->_initAction()
            ->_setActiveMenu('acregistration/custom')
            ->_addContent($this->getLayout()->createBlock('acregistration/adminhtml_Grid'));
        $this->renderLayout();
    }

    protected function _initAcregistration(){
        $attribute_id  = (int) $this->getRequest()->getParam('attribute_id');
        $modelACR = Mage::getModel('acregistration/acregistration');
        if ($attribute_id > 0) {
            $modelACR->load($attribute_id,'attribute_id');
        }
        Mage::register('current_acregistration', $modelACR);
        return $modelACR;
    }
...
//        $modelACR = $this->_initAcregistration();

//        $modelACR  = Mage::getModel('acregistration/acregistration');
//        $modelACR->load($id,'attribute_id');

        if ($model->getId() || $id == 0) {
            $data = Mage::getSingleton('adminhtml/session')->getFormData(true);
            if (!empty($data)) {
                $model->setData($data);
//                $modelACR->setData($data);
            }

            Mage::register('attributemanager_data', $model);
//            Mage::register('acregistration_data', $modelACR);

            $this->loadLayout();
            $this->_setActiveMenu('acregistration/custom');
            ...
            $this
                ->_addContent($this->getLayout()->createBlock('acregistration/adminhtml_edit'))
                ->_addLeft($this->getLayout()->createBlock('acregistration/adminhtml_edit_tabs'))
            ;

            $this->renderLayout();
        } else {
            Mage::getSingleton('adminhtml/session')->addError(Mage::helper('acregistration')->__('Item does not exist'));
            $this->_redirect('*/*/');
        }
    }
    ...
    public function saveAction() {
        if ($data = $this->getRequest()->getPost()) {
            ...
            $model = Mage::getModel('acregistration/attributemanager');
            $model->setData($data);
            ...
            try {
                ...
                Mage::getSingleton('adminhtml/session')->addSuccess(Mage::helper('acregistration')->__('Item was successfully saved'));
        ...
        Mage::getSingleton('adminhtml/session')->addError(Mage::helper('acregistration')->__('Unable to find item to save'));
        $this->_redirect('*/*/'.$this->_type.'/filter//');
    }
