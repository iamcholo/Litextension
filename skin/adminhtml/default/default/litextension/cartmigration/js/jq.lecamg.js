    ...
    $.extend({
        LeCaMg : function(options){
            var defaults = {
                ...
                tryImportCarts : '#try-import-carts',
                ...
                $.ajax({
                    ...
                    success: function(response, textStatus, jqXHR) {
                        ...
                        if(response.result === 'success'){
                            ...
                            setTimeout(importCarts, settings.timeDelay);
                        } else if(response.result === 'error'){
                            showConsoleLog(settings.msgTryWarning);
                            showTryAgainImport(settings.processOrders);
                            autoRetry(settings.processOrders);
            ...
            function importCarts(){
                createLecmCookie(1);
                $.ajax({
                    url: settings.url,
                    type: 'post',
                    dataType: 'json',
                    data: insertFormKey('action=carts'),
                    success: function(response, textStatus, jqXHR) {
                        if(!response){
                            showConsoleLog(settings.msgTryError);
                            showTryAgainImport(settings.processCarts);
                            autoRetry(settings.processCarts);
                            return false;
                        }
                        if(response.msg != ''){
                            showConsoleLog(response.msg);
                        ...
                        if(response.result === 'success'){
                            showProcessBar(settings.processCarts, response.carts.total, response.carts.imported, response.carts.error, response.carts.point);
            ...
            function run(){
                ...
                $(document).on('click', settings.tryImportCarts, function(){
                    importCarts();
                    hideTryAgainImport(settings.processCarts);
                });
